...
tree require_complete_type(tree value)
{
    ...
    if (TYPE_SIZE(type) != 0 && type != void_type_node)
        return value;
...
void incomplete_type_error(tree value, tree type)
{
    ...
    if (value != 0 && (TREE_CODE(value) == VAR_DECL || TREE_CODE(value) == PARM_DECL))
        error("`%s' has an incomplete type", IDENTIFIER_POINTER(DECL_NAME(value)));
    else
    {
    retry:
       ...
       So I turned it on again.  */
        {
            ...
            tree target
                = common_type(TYPE_MAIN_VARIANT(pointed_to_1), TYPE_MAIN_VARIANT(pointed_to_2));
            t1 = build_pointer_type(c_build_qualified_type(
                target, TYPE_QUALS(pointed_to_1) | TYPE_QUALS(pointed_to_2)));
            return build_type_attribute_variant(t1, attributes);
        }
#if 0
       ...
       If both do, merge the arg types.  Also merge the return types.  */
        {
            ...
            for (; p1; p1 = TREE_CHAIN(p1), p2 = TREE_CHAIN(p2), n = TREE_CHAIN(n))
            {
                ...
                {
                    tree memb;
                    for (memb = TYPE_FIELDS(TREE_VALUE(p1)); memb; memb = TREE_CHAIN(memb))
                ...
                {
                    tree memb;
                    for (memb = TYPE_FIELDS(TREE_VALUE(p2)); memb; memb = TREE_CHAIN(memb))
...
#define COMP_TYPE_ATTRIBUTES(t1, t2) 1
    ...
    {
    case POINTER_TYPE:
        val = (TREE_TYPE(t1) == TREE_TYPE(t2) ? 1 : comptypes(TREE_TYPE(t1), TREE_TYPE(t2)));
        break;
    ...
    case ARRAY_TYPE:
    {
        ...
        if (TREE_TYPE(t1) != TREE_TYPE(t2) && 0 == (val = comptypes(TREE_TYPE(t1), TREE_TYPE(t2))))
            return 0;

        /* Sizes must match unless one is missing or variable.  */
        if (d1 == 0 || d2 == 0 || d1 == d2 || TREE_CODE(TYPE_MIN_VALUE(d1)) != INTEGER_CST
            || TREE_CODE(TYPE_MIN_VALUE(d2)) != INTEGER_CST
            || TREE_CODE(TYPE_MAX_VALUE(d1)) != INTEGER_CST
            || TREE_CODE(TYPE_MAX_VALUE(d2)) != INTEGER_CST)
            break;

        if (!((TREE_INT_CST_LOW(TYPE_MIN_VALUE(d1)) == TREE_INT_CST_LOW(TYPE_MIN_VALUE(d2)))
                && (TREE_INT_CST_HIGH(TYPE_MIN_VALUE(d1)) == TREE_INT_CST_HIGH(TYPE_MIN_VALUE(d2)))
                && (TREE_INT_CST_LOW(TYPE_MAX_VALUE(d1)) == TREE_INT_CST_LOW(TYPE_MAX_VALUE(d2)))
...
static int comp_target_types(tree ttl, tree ttr)
{
    int val;

    val = comptypes(TYPE_MAIN_VARIANT(TREE_TYPE(ttl)), TYPE_MAIN_VARIANT(TREE_TYPE(ttr)));
...
static int function_types_compatible_p(tree f1, tree f2)
{
    ...
    if (!(TREE_TYPE(f1) == TREE_TYPE(f2) || (val = comptypes(TREE_TYPE(f1), TREE_TYPE(f2)))))
        return 0;
       ...
       So match anything that self-promotes.  */
        ...
        else if (!(newval = comptypes(TREE_VALUE(args1), TREE_VALUE(args2))))
        {
            ...
            if (TREE_CODE(TREE_VALUE(args1)) == UNION_TYPE
                && (TYPE_NAME(TREE_VALUE(args1)) == 0 || TYPE_TRANSPARENT_UNION(TREE_VALUE(args1)))
                && TREE_CODE(TYPE_SIZE(TREE_VALUE(args1))) == INTEGER_CST
                && tree_int_cst_equal(TYPE_SIZE(TREE_VALUE(args1)), TYPE_SIZE(TREE_VALUE(args2))))
            {
                tree memb;
                for (memb = TYPE_FIELDS(TREE_VALUE(args1)); memb; memb = TREE_CHAIN(memb))
                ...
                && (TYPE_NAME(TREE_VALUE(args2)) == 0 || TYPE_TRANSPARENT_UNION(TREE_VALUE(args2)))
                && TREE_CODE(TYPE_SIZE(TREE_VALUE(args2))) == INTEGER_CST
                && tree_int_cst_equal(TYPE_SIZE(TREE_VALUE(args2)), TYPE_SIZE(TREE_VALUE(args1))))
            {
                tree memb;
                for (memb = TYPE_FIELDS(TREE_VALUE(args2)); memb; memb = TREE_CHAIN(memb))
...
static int self_promoting_type_p(tree type)
{
    if (TYPE_MAIN_VARIANT(type) == float_type_node)
        return 0;
...
{
    if ((!INTEGRAL_TYPE_P(type) && !POINTER_TYPE_P(type)) || TREE_UNSIGNED(type) == unsignedp)
        return type;
...
tree c_alignof_expr(tree expr)
{
    ...
    if (TREE_CODE(expr) == COMPONENT_REF && DECL_C_BIT_FIELD(TREE_OPERAND(expr, 1)))
    {
        error("`__alignof' applied to a bit-field");
        return size_int(1);
    }
    else if (TREE_CODE(expr) == COMPONENT_REF && TREE_CODE(TREE_OPERAND(expr, 1)) == FIELD_DECL)
...
static tree decl_constant_value(tree decl)
{
    if (/* Don't change a variable array bound or initial value to a constant
       in a place where a variable is invalid.  */
        current_function_decl != 0 && !pedantic && !TREE_THIS_VOLATILE(decl) && TREE_READONLY(decl)
        && !ITERATOR_P(decl) && DECL_INITIAL(decl) != 0
       ...
       or a variable, then re-evaluating it could give different results.  */
        ...
        && TREE_CODE(DECL_INITIAL(decl)) != CONSTRUCTOR && DECL_MODE(decl) != BLKmode)
...
tree default_conversion(tree exp)
{
    ...
    while (TREE_CODE(exp) == NON_LVALUE_EXPR
        || (TREE_CODE(exp) == NOP_EXPR && TREE_TYPE(TREE_OPERAND(exp, 0)) == TREE_TYPE(exp)))
    ...
    if (TREE_CODE(exp) == COMPONENT_REF && DECL_C_BIT_FIELD(TREE_OPERAND(exp, 1)))
    {
        tree width = DECL_SIZE(TREE_OPERAND(exp, 1));
        int32_t low = TREE_INT_CST_LOW(width);
    ...
    if (code == ARRAY_TYPE)
    {
        ...
        if (TREE_CODE_CLASS(TREE_CODE(exp)) == 'r' || TREE_CODE_CLASS(TREE_CODE(exp)) == 'd')
        {
        ...
        if (TYPE_QUALS(type) || constp || volatilep)
            restype = c_build_qualified_type(restype,
                TYPE_QUALS(type) | (constp * TYPE_QUAL_CONST) | (volatilep * TYPE_QUAL_VOLATILE));

        if (TREE_CODE(exp) == INDIRECT_REF)
            return convert(TYPE_POINTER_TO(restype), TREE_OPERAND(exp, 0));
...
static tree lookup_field(tree type, tree component, tree *indirect)
{
    tree field;
...
tree build_component_ref(tree datum, tree component)
{
    ...
    case COND_EXPR:
        return build_conditional_expr(TREE_OPERAND(datum, 0),
    ...
    if (code == RECORD_TYPE || code == UNION_TYPE)
    {
        ...
        if (!field)
        {
            error(code == RECORD_TYPE ? "structure has no member named `%s'"
...
tree build_indirect_ref(tree ptr, const char *errorstring)
{
    ...
    {
        if (TREE_CODE(pointer) == ADDR_EXPR && !flag_volatile
            && (TREE_TYPE(TREE_OPERAND(pointer, 0)) == TREE_TYPE(type)))
...
tree build_array_ref(tree array, tree index)
{
    ...
    if (TREE_TYPE(array) == error_mark_node || TREE_TYPE(index) == error_mark_node)
        return error_mark_node;
    ...
    if (TREE_CODE(TREE_TYPE(array)) == ARRAY_TYPE && TREE_CODE(array) != INDIRECT_REF)
    {
       ...
       must have done so deliberately.  */
        if (warn_char_subscripts && TYPE_MAIN_VARIANT(TREE_TYPE(index)) == char_type_node)
       ...
       to access a non-existent part of the register.  */
        if (TREE_CODE(index) == INTEGER_CST && TYPE_VALUES(TREE_TYPE(array))
...

    {
        ...
        /* Do the same warning check as above, but only on the part that's
           ...
           the index.  */
        if (warn_char_subscripts && TREE_CODE(TREE_TYPE(index)) == INTEGER_TYPE
        ...
        return build_indirect_ref(build_binary_op(PLUS_EXPR, ar, ind, 0), "array indexing");
    }
}
...
tree build_function_call(tree function, tree params)
{
    ...
    if (TREE_CODE(function) == FUNCTION_DECL)
    {
       ...
       needs to be separately compiled).  */
        fntype = build_type_variant(
            TREE_TYPE(function), TREE_READONLY(function), TREE_THIS_VOLATILE(function));
        fundecl = function;
    ...
    coerced_params = convert_arguments(TYPE_ARG_TYPES(fntype), params, name, fundecl);
...
static tree convert_arguments(tree typelist, tree values, tree name, tree fundecl)
{
         ...
         valtail = TREE_CHAIN(valtail), parmnum++)
    {
        ...
        {
            if (name)
                error("too many arguments to function `%s'", IDENTIFIER_POINTER(name));
            else
       ...
       to convert automatically to a pointer.  */
        ...
        if (TREE_CODE(TREE_TYPE(val)) == ARRAY_TYPE || TREE_CODE(TREE_TYPE(val)) == FUNCTION_TYPE)
        ...
        if (type != 0)
        {
            ...
            else
            {
               ...
               differ from the default conversions.  */
                if (warn_conversion)
                {
                    ...
                    if (INTEGRAL_TYPE_P(type) && TREE_CODE(TREE_TYPE(val)) == REAL_TYPE)
                        ...
                        warn_for_assignment("%s as complex rather than floating due to prototype",
                            (char *)0, name, parmnum + 1);
                    else if (TREE_CODE(type) == REAL_TYPE && INTEGRAL_TYPE_P(TREE_TYPE(val)))
                    ...
                    /* ??? At some point, messages should be written about
                       ...
                       to do now.  */
                    else if (TREE_CODE(type) == REAL_TYPE && TREE_CODE(TREE_TYPE(val)) == REAL_TYPE)
                    {
                        /* Warn if any argument is passed as `float',
                       since without a prototype it would be `double'.  */
                        if (formal_prec == TYPE_PRECISION(float_type_node))
                            warn_for_assignment(
                                "%s as `float' rather than `double' due to prototype", (char *)0,
                                name, parmnum + 1);
                    }
                    /* Detect integer changing in width or signedness.  */
                    else if (INTEGRAL_TYPE_P(type) && INTEGRAL_TYPE_P(TREE_TYPE(val)))
                    {
                        tree would_have_been = default_conversion(val);
                        tree type1 = TREE_TYPE(would_have_been);

                        if (TREE_CODE(type) == ENUMERAL_TYPE && type == TREE_TYPE(val))
                            /* No warning if function asks for enum
                               and the actual arg is that enum type.  */
                            ;
                        else if (formal_prec != TYPE_PRECISION(type1))
                            warn_for_assignment("%s with different width due to prototype",
                                (char *)0, name, parmnum + 1);
			...
			;
                       ...
                       certainly is the same either way.  */
                            ...
                            warn_for_assignment(
                                "%s as unsigned due to prototype", (char *)0, name, parmnum + 1);
                        else
                            warn_for_assignment(
                                "%s as signed due to prototype", (char *)0, name, parmnum + 1);
                    }
                }

                parmval = convert_for_assignment(type, val, (char *)0, /* arg passing  */
    ...
    {
        if (name)
            error("too few arguments to function `%s'", IDENTIFIER_POINTER(name));
        else
...
tree parser_build_binary_op(enum tree_code code, tree arg1, tree arg2)
{
    ...
    if (class1 == 'e' || class1 == '1' || class1 == '2' || class1 == '<')
        code1 = C_EXP_ORIGINAL_CODE(arg1);
    if (class2 == 'e' || class2 == '1' || class2 == '2' || class2 == '<')
    ...
    if (warn_parentheses)
    {
        ...
        {
            if (code1 == PLUS_EXPR || code1 == MINUS_EXPR || code2 == PLUS_EXPR
                || code2 == MINUS_EXPR)
        ...
        {
            if (code1 == TRUTH_ANDIF_EXPR || code2 == TRUTH_ANDIF_EXPR)
                warning("suggest parentheses around && within ||");
        }

        if (code == BIT_IOR_EXPR)
        {
            if (code1 == BIT_AND_EXPR || code1 == BIT_XOR_EXPR || code1 == PLUS_EXPR
                || code1 == MINUS_EXPR || code2 == BIT_AND_EXPR || code2 == BIT_XOR_EXPR
        ...
        {
            if (code1 == BIT_AND_EXPR || code1 == PLUS_EXPR || code1 == MINUS_EXPR
                || code2 == BIT_AND_EXPR || code2 == PLUS_EXPR || code2 == MINUS_EXPR)
                warning("suggest parentheses around arithmetic in operand of ^");
            /* Check cases like x^y==z */
            if (TREE_CODE_CLASS(code1) == '<' || TREE_CODE_CLASS(code2) == '<')
                warning("suggest parentheses around comparison in operand of ^");
...
tree build_binary_op(enum tree_code code, tree orig_op0, tree orig_op1, int convert_p)
{
    ...
    case BIT_XOR_EXPR:
       ...
       in the short type, making the entire operation go away.  */
        if (TREE_CODE(op0) == INTEGER_CST && TREE_CODE(op1) == NOP_EXPR
        ...
        if (TREE_CODE(op1) == INTEGER_CST && TREE_CODE(op0) == NOP_EXPR
            ...
            if (TREE_CODE(op1) == INTEGER_CST && skip_evaluation == 0)
            {
                ...
                else
                {
                    ...
                    if (TREE_INT_CST_HIGH(op1) != 0
                        || ((uint32_t)TREE_INT_CST_LOW(op1) >= TYPE_PRECISION(type0)))
    ...
    case NE_EXPR:
       ...
       but don't convert the args to int!  */
        ...
        else if (code0 == POINTER_TYPE && TREE_CODE(op1) == INTEGER_CST && integer_zerop(op1))
            result_type = type0;
        else if (code1 == POINTER_TYPE && TREE_CODE(op0) == INTEGER_CST && integer_zerop(op0))
            result_type = type1;
        else if (code0 == POINTER_TYPE && code1 == INTEGER_TYPE)
        {
            result_type = type0;
    ...
    case MIN_EXPR:
        ...
        else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
        {
            ...
            {
                result_type = common_type(type0, type1);
                if (pedantic && TREE_CODE(TREE_TYPE(type0)) == FUNCTION_TYPE)
    ...
    case GT_EXPR:
        ...
        else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
        {
            ...
            {
                result_type = common_type(type0, type1);
                if ((TYPE_SIZE(TREE_TYPE(type0)) != 0) != (TYPE_SIZE(TREE_TYPE(type1)) != 0))
               ...
               when it is extended.  */
                ...
                if ((TREE_CODE(primop0) == BIT_NOT_EXPR) != (TREE_CODE(primop1) == BIT_NOT_EXPR))
                {
                    ...
                    if (TREE_CODE(primop0) == INTEGER_CST || TREE_CODE(primop1) == INTEGER_CST)
                    {
                        ...
                        if (bits < TYPE_PRECISION(result_type) && bits < 32
                            && unsignedp)
                        {
                            mask = (~((int32_t)0)) << bits;
...
static tree pointer_int_sum(enum tree_code resultcode, tree ptrop, tree intop)
{
    ...
    if ((TREE_CODE(intop) == PLUS_EXPR || TREE_CODE(intop) == MINUS_EXPR) && !TREE_CONSTANT(intop)
       ...
       an overflow error if the constant is negative but INTOP is not.  */
        && (!TREE_UNSIGNED(TREE_TYPE(intop))
               || (TYPE_PRECISION(TREE_TYPE(intop)) == TYPE_PRECISION(TREE_TYPE(ptrop)))))
    {
       ...
       can result in a sum or difference with different type args.  */
        ptrop = build_binary_op(subcode, ptrop, convert(int_type, TREE_OPERAND(intop, 1)), 1);
    ...
    if (TYPE_PRECISION(TREE_TYPE(intop)) != TYPE_PRECISION(sizetype)
        ...
        intop = convert(type_for_size(TYPE_PRECISION(sizetype), TREE_UNSIGNED(sizetype)), intop);
    ...
    intop = convert(
        result_type, build_binary_op(MULT_EXPR, intop, convert(TREE_TYPE(intop), size_exp), 1));
...
static tree pointer_diff(tree op0, tree op1)
{
    ...
    op0 = build_binary_op(MINUS_EXPR, convert(restype, op0), convert(restype, op1), 0);
...
tree build_unary_op(enum tree_code code, tree xarg, int noconvert)
{
    ...
    case TRUTH_NOT_EXPR:
        if (typecode != INTEGER_TYPE && typecode != REAL_TYPE && typecode != POINTER_TYPE
    ...
    case POSTDECREMENT_EXPR:
       ...
       and don't change the imaginary part.  */
        if (typecode == COMPLEX_TYPE)
        {
            ...
            return build(COMPLEX_EXPR, TREE_TYPE(arg), build_unary_op(code, real, 1), imag);
        }

        /* Report invalid types.  */

        if (typecode != POINTER_TYPE && typecode != INTEGER_TYPE && typecode != REAL_TYPE)
        {
            ...
            while (1)
                ...
                case FIX_CEIL_EXPR:
                   ...
                   by adding directly to the inside of the cast.  */
                    if ((TREE_CODE(TREE_TYPE(arg)) == TREE_CODE(TREE_TYPE(TREE_OPERAND(arg, 0))))
       ...
       regardless of NOCONVERT.  */
        ...
        if (TREE_CODE(arg) == ARRAY_REF)
        {
            if (mark_addressable(TREE_OPERAND(arg, 0)) == 0)
                return error_mark_node;
            return build_binary_op(PLUS_EXPR, TREE_OPERAND(arg, 0), TREE_OPERAND(arg, 1), 1);
        }
	...
	case FIX_CEIL_EXPR:
	  ...
	  return convert (build_pointer_type (TREE_TYPE (arg)),
       ...
       only have to deal with `const' and `volatile' here.  */
        if (TREE_CODE_CLASS(TREE_CODE(arg)) == 'd' || TREE_CODE_CLASS(TREE_CODE(arg)) == 'r')
        {
            if (TREE_READONLY(arg) || TREE_THIS_VOLATILE(arg))
                argtype
                    = c_build_type_variant(argtype, TREE_READONLY(arg), TREE_THIS_VOLATILE(arg));
        }
...

        {
            ...
            if (TREE_CODE(arg) == COMPONENT_REF)
            {
                ...
                {
                    tree offset = size_binop(
                        EASY_DIV_EXPR, DECL_FIELD_BITPOS(field), size_int(BITS_PER_UNIT));
            ...
            if (staticp(arg) && !(TREE_CODE(arg) == FUNCTION_DECL && DECL_CONTEXT(arg) != 0))
...
int lvalue_p(tree ref)
{
    ...
    case ERROR_MARK:
        return (
            TREE_CODE(TREE_TYPE(ref)) != FUNCTION_TYPE && TREE_CODE(TREE_TYPE(ref)) != METHOD_TYPE);
...
static tree unary_complex_lvalue(enum tree_code code, tree arg)
{
    ...
    if (TREE_CODE(arg) == COND_EXPR)
    {
        ...
        return (build_conditional_expr(TREE_OPERAND(arg, 0),
...
static void pedantic_lvalue_warning(enum tree_code code)
{
    if (pedantic)
        pedwarn("ANSI C forbids use of %s expressions as lvalues",
            code == COND_EXPR ? "conditional" : code == COMPOUND_EXPR ? "compound" : "cast");
}
...
tree build_conditional_expr(tree ifexp, tree op1, tree op2)
{
  ...
  if (TREE_TYPE (op1) == TREE_TYPE (op2)
    ...
    if (TREE_CODE(ifexp) == ERROR_MARK || TREE_CODE(TREE_TYPE(op1)) == ERROR_MARK
    ...
    result_type = build_type_variant(result_type,
...
static tree internal_build_compound_expr(tree list, int first_p)
{
    ...
    if (TREE_CHAIN(list) != 0 && TREE_CHAIN(TREE_CHAIN(list)) == 0)
    {
        ...
        if (TREE_CODE(TREE_TYPE(TREE_VALUE(TREE_CHAIN(list)))) == ARRAY_TYPE)
            TREE_VALUE(TREE_CHAIN(list)) = default_conversion(TREE_VALUE(TREE_CHAIN(list)));
    }
...
tree build_c_cast(tree type, tree expr)
{
  ...
  if (TREE_CODE (value) == NON_LVALUE_EXPR)
    ...
    else if (TREE_CODE(type) == UNION_TYPE)
    {
        ...
        for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field))
            if (comptypes(TYPE_MAIN_VARIANT(TREE_TYPE(field)), TYPE_MAIN_VARIANT(TREE_TYPE(value))))
                break;
        ...
        if (field)
        {
            ...
            if (TYPE_NAME(type) != 0)
            {
                ...
                name = "";
            t = digest_init(
                type, build(CONSTRUCTOR, type, NULL_TREE, build_tree_list(field, value)), 0, 0);
    ...
    else
    {
       ...
       but don't convert any other types.  */
        ...
        if (STRICT_ALIGNMENT && warn_cast_align && TREE_CODE(type) == POINTER_TYPE
            && TREE_CODE(otype) == POINTER_TYPE && TREE_CODE(TREE_TYPE(otype)) != VOID_TYPE
        ...
        if (TREE_CODE(type) == INTEGER_TYPE && TREE_CODE(otype) == POINTER_TYPE
            && TYPE_PRECISION(type) != TYPE_PRECISION(otype) && !TREE_CONSTANT(value))
            warning("cast from pointer to integer of different size");

        if (warn_bad_function_cast && TREE_CODE(value) == CALL_EXPR
    ...
    if (pedantic && TREE_CODE(value) == INTEGER_CST && TREE_CODE(expr) == INTEGER_CST
...
tree build_modify_expr(tree lhs, enum tree_code modifycode, tree rhs)
{
    ...
    case COND_EXPR:
        ...
        rhs = save_expr(rhs);
        {
            ...
            tree cond = build_conditional_expr(TREE_OPERAND(lhs, 0),
    ...
    case FIX_CEIL_EXPR:
            ...
            newrhs = default_conversion(newrhs);
        {
            ...
            result = build_modify_expr(
                inner_lhs, NOP_EXPR, convert(TREE_TYPE(inner_lhs), convert(lhstype, newrhs)));
    ...
    if (TREE_READONLY(lhs) || TYPE_READONLY(lhstype)
        || ((TREE_CODE(lhstype) == RECORD_TYPE || TREE_CODE(lhstype) == UNION_TYPE)
    ...
    newrhs = convert_for_assignment(lhstype, newrhs, "assignment", NULL_TREE, NULL_TREE, 0);
    if (TREE_CODE(newrhs) == ERROR_MARK)
        return error_mark_node;
    ...
    if (olhstype == TREE_TYPE(result))
        return result;
    return convert_for_assignment(olhstype, result, "assignment", NULL_TREE, NULL_TREE, 0);
}
    ...
    tree type, tree rhs, const char *errtype, tree fundecl, tree funname, int parmnum)
{
    ...
    if (TREE_CODE(TREE_TYPE(rhs)) == ARRAY_TYPE || TREE_CODE(TREE_TYPE(rhs)) == FUNCTION_TYPE)
    ...
    else if (codel == UNION_TYPE && TYPE_TRANSPARENT_UNION(type) && !errtype)
    {
        ...
        for (memb_types = TYPE_FIELDS(type); memb_types; memb_types = TREE_CHAIN(memb_types))
        {
            tree memb_type = TREE_TYPE(memb_types);

            if (comptypes(TYPE_MAIN_VARIANT(memb_type), TYPE_MAIN_VARIANT(rhstype)))
                break;
               ...
               the rhs.  */
                    ...
                    || comp_target_types(memb_type, rhstype))
                {
                    ...
                    if (TYPE_QUALS(ttl) == TYPE_QUALS(ttr)
                        || ((TREE_CODE(ttr) == FUNCTION_TYPE && TREE_CODE(ttl) == FUNCTION_TYPE)
                                   ? ((TYPE_QUALS(ttl) | TYPE_QUALS(ttr)) == TYPE_QUALS(ttr))
                                   : ((TYPE_QUALS(ttl) | TYPE_QUALS(ttr)) == TYPE_QUALS(ttl))))
                        break;
            ...
            if (integer_zerop(rhs)
                || (TREE_CODE(rhs) == NOP_EXPR && integer_zerop(TREE_OPERAND(rhs, 0))))
            {
    ...
    else if (codel == POINTER_TYPE && coder == POINTER_TYPE)
    {
       ...
       Meanwhile, the lhs target must have all the qualifiers of the rhs.  */
        if (TYPE_MAIN_VARIANT(ttl) == void_type_node || TYPE_MAIN_VARIANT(ttr) == void_type_node
            || comp_target_types(type, rhstype)
            || (unsigned_type(TYPE_MAIN_VARIANT(ttl)) == unsigned_type(TYPE_MAIN_VARIANT(ttr))))
        {
            if (pedantic
                && ((TYPE_MAIN_VARIANT(ttl) == void_type_node && TREE_CODE(ttr) == FUNCTION_TYPE)
                       || (TYPE_MAIN_VARIANT(ttr) == void_type_node
            ...
            else if (TREE_CODE(ttr) != FUNCTION_TYPE && TREE_CODE(ttl) != FUNCTION_TYPE)
            {
               ...
               no warning.  */
                    ...
                    || TYPE_MAIN_VARIANT(ttr) == void_type_node || comp_target_types(type, rhstype))
                    ;
            ...
            warn_for_assignment(
                "%s from incompatible pointer type", get_spelling(errtype), funname, parmnum);
    ...
    else if (codel == POINTER_TYPE && coder == INTEGER_TYPE)
    {
       ...
       a cast to integer type.  */
        if (!(TREE_CODE(rhs) == INTEGER_CST && integer_zerop(rhs))
            && !(TREE_CODE(rhs) == NOP_EXPR && TREE_CODE(TREE_TYPE(rhs)) == INTEGER_TYPE
    ...
    {
        warn_for_assignment("%s makes integer from pointer without a cast", get_spelling(errtype),
            funname, parmnum);
        return convert(type, rhs);
    }
    ...
    if (!errtype)
    {
        ...
        {
            error(
                "incompatible type for argument %d of `%s'", parmnum, IDENTIFIER_POINTER(funname));
        }
        else
...
tree initializer_constant_valid_p(tree value, tree endtype)
{
    ...
    case CONSTRUCTOR:
        if ((TREE_CODE(TREE_TYPE(value)) == UNION_TYPE
            ...
            && TREE_CONSTANT(value) && CONSTRUCTOR_ELTS(value))
    ...
    case PLUS_EXPR:
        if (TREE_CODE(endtype) == INTEGER_TYPE && TYPE_PRECISION(endtype) < POINTER_SIZE)
            return 0;
        {
...
static tree valid_compound_expr_initializer(tree value, tree endtype)
{
    ...
    {
        if (valid_compound_expr_initializer(TREE_OPERAND(value, 0), endtype) == error_mark_node)
            return error_mark_node;
        return valid_compound_expr_initializer(TREE_OPERAND(value, 1), endtype);
    }
    else if (!TREE_CONSTANT(value) && !initializer_constant_valid_p(value, endtype))
        return error_mark_node;
    else
        return value;
}
...
void store_init_value(tree decl, tree init)
{
    ...
    value = digest_init(type, init, TREE_STATIC(decl), TREE_STATIC(decl) || pedantic);
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            ...
            if (spelling_base == 0)                                                                \
                ...
                spelling_base = (struct spelling *)xrealloc(                                       \
                    spelling_base, spelling_size * sizeof(struct spelling));                       \
...
static tree digest_init(tree type, tree init, int require_constant, int constructor_constant)
{
    enum tree_code code = TREE_CODE(type);
    ...
    if (code == ARRAY_TYPE)
    {
        ...
        if ((typ1 == char_type_node || typ1 == signed_char_type_node
                || typ1 == unsigned_char_type_node || typ1 == unsigned_wchar_type_node
            ...
            && ((inside_init && TREE_CODE(inside_init) == STRING_CST)))
        {
            ...
            {
                error_init("char-array%s initialized from wide string", " `%s'", NULL);
                return error_mark_node;
            }
            if ((TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(inside_init))) == char_type_node)
                && TYPE_PRECISION(typ1) != TYPE_PRECISION(char_type_node))
            {
                error_init("int-array%s initialized from non-wide string", " `%s'", NULL);
                return error_mark_node;
            }
               ...
               that is counted in the length of the constant.  */
                if (size < TREE_STRING_LENGTH(inside_init)
                        - (TYPE_PRECISION(typ1) != TYPE_PRECISION(char_type_node)
                                  ...
                                  : 1))
                    pedwarn_init(
                        "initializer-string for array of chars%s is too long", " `%s'", NULL);
            }
            return inside_init;
        }
    }
                      ...
                      && comptypes(TREE_TYPE(TREE_TYPE(inside_init)), TREE_TYPE(type)))))
    {
        ...
        {
            error_init("array%s initialized from non-constant array expression", " `%s'", NULL);
            return error_mark_node;
        }
       ...
       an error.  In the former case, we simply want a warning.  */
        if (require_constant && pedantic && TREE_CODE(inside_init) == COMPOUND_EXPR)
        {
            ...
            if (inside_init == error_mark_node)
                error_init("initializer element%s is not constant", " for `%s'", NULL);
            else
                pedwarn_init("initializer element%s is not constant", " for `%s'", NULL);
        ...
        {
            error_init("initializer element%s is not computable at load time", " for `%s'", NULL);
            inside_init = error_mark_node;
        }
        ...
        || code == COMPLEX_TYPE || code == BOOLEAN_TYPE)
    {
       ...
       case where inside_init is a null pointer constant.  */
        inside_init = convert_for_assignment(type, init, "initialization", NULL_TREE, NULL_TREE, 0);
    ...
    {
        error_init("variable-sized object%s may not be initialized", " `%s'", NULL);
        return error_mark_node;
    }
    ...
    if (flag_traditional)
    {
        ...
        while (TREE_CODE(type) == RECORD_TYPE || TREE_CODE(type) == ARRAY_TYPE
            || TREE_CODE(type) == QUAL_UNION_TYPE || TREE_CODE(type) == UNION_TYPE)
        {
        ...
        if (otype != type)
        {
            TREE_OPERAND(prev, 1) = build_tree_list(
                NULL_TREE, digest_init(type, init, require_constant, constructor_constant));
            return top;
        }
        else
            return error_mark_node;
...
void really_start_incremental_init(tree type)
{
    ...
    if (TREE_CODE(constructor_type) == RECORD_TYPE || TREE_CODE(constructor_type) == UNION_TYPE)
    {
    ...
    else if (TREE_CODE(constructor_type) == ARRAY_TYPE)
    {
        ...
        {
            constructor_max_index = TYPE_MAX_VALUE(TYPE_DOMAIN(constructor_type));
            constructor_index = copy_node(TYPE_MIN_VALUE(TYPE_DOMAIN(constructor_type)));
        }
        else
    ...
    if (constructor_incremental)
    {
        ...
        make_decl_rtl(constructor_decl, constructor_asmspec, constructor_top_level);
...
void push_init_level(int implicit)
{
        ...
        && constructor_fields == constructor_unfilled_fields)
    {
        ...
        if (!tree_int_cst_equal(constructor_bit_index, DECL_FIELD_BITPOS(constructor_fields)))
        {
            /* By using unsigned arithmetic, the result will be correct even
               in case of overflows, if BITS_PER_UNIT is a power of two.  */
            unsigned next = (TREE_INT_CST_LOW(DECL_FIELD_BITPOS(constructor_fields))
                / (unsigned)BITS_PER_UNIT);
            unsigned here = (TREE_INT_CST_LOW(constructor_bit_index) / (unsigned)BITS_PER_UNIT);

            assemble_zeros((next - here) * (unsigned)BITS_PER_UNIT / (unsigned)BITS_PER_UNIT);
        }
    ...
    if (constructor_type == 0)
    {
        error_init("extra brace group at end of initializer%s", " for `%s'", NULL);
        constructor_fields = 0;
...
static void check_init_type_bitfields(tree type)
{
...
tree pop_init_level(int implicit)
{
    ...
    if (extra_warnings && constructor_type && TREE_CODE(constructor_type) == RECORD_TYPE
  ...
  if (! implicit && pedantic
    ...
    if (p->replacement_value)
    {
       ...
       assemble_variable, we must output the value now.  */
        if (p->next == 0 && constructor_decl != 0 && constructor_incremental)
        {
            constructor = digest_init(
                constructor_type, constructor, require_constant_value, require_constant_elements);

            /* If initializing an array of unknown size,
               determine the size now.  */
            if (TREE_CODE(constructor_type) == ARRAY_TYPE && TYPE_DOMAIN(constructor_type) == 0)
            {
                int failure;
                int momentary_p;
               ...
               some other type.  */
                ...
                failure = complete_array_type(constructor_type, constructor, 0);
                if (failure)
                    abort();
        ...
        ;
    else if (TREE_CODE(constructor_type) != RECORD_TYPE && TREE_CODE(constructor_type) != UNION_TYPE
        && TREE_CODE(constructor_type) != ARRAY_TYPE && !constructor_incremental)
    {
        /* A nonincremental scalar initializer--just return
       the element, after verifying there is just one.  */
        if (constructor_elements == 0)
        {
            error_init("empty scalar initializer%s", " for `%s'", NULL);
    ...
    else if (!constructor_incremental)
    {
        ...
        else
        {
            ...
            constructor
                = build(CONSTRUCTOR, constructor_type, NULL_TREE, nreverse(constructor_elements));
    ...
    else
    {
        ...
        if (TREE_CODE(constructor_type) == RECORD_TYPE || TREE_CODE(constructor_type) == UNION_TYPE)
        {
            ...
            filled = size_binop(CEIL_DIV_EXPR, constructor_bit_index, size_int(BITS_PER_UNIT));
        }
        else if (TREE_CODE(constructor_type) == ARRAY_TYPE)
        {
            /* If initializing an array of unknown size,
            ...
            {
                tree maxindex
                    = size_binop(MINUS_EXPR, constructor_unfilled_index, integer_one_node);
               ...
               warn only if the value is less than zero.  */
                if (pedantic
                    && (tree_int_cst_sgn(TYPE_MAX_VALUE(TYPE_DOMAIN(constructor_type))) < 0))
                    error_with_decl(constructor_decl, "zero or negative array size `%s'");
            ...
            filled = size_binop(
                MULT_EXPR, constructor_unfilled_index, size_in_bytes(TREE_TYPE(constructor_type)));
        }
        else
            filled = 0;
...
{
    while ((TREE_CODE(first) == NOP_EXPR || TREE_CODE(first) == CONVERT_EXPR
               || TREE_CODE(first) == NON_LVALUE_EXPR)
        && (TYPE_MODE(TREE_TYPE(first)) == TYPE_MODE(TREE_TYPE(TREE_OPERAND(first, 0)))))
        (first) = TREE_OPERAND(first, 0);
    if (last)
        while ((TREE_CODE(last) == NOP_EXPR || TREE_CODE(last) == CONVERT_EXPR
                   || TREE_CODE(last) == NON_LVALUE_EXPR)
            && (TYPE_MODE(TREE_TYPE(last)) == TYPE_MODE(TREE_TYPE(TREE_OPERAND(last, 0)))))
...
void set_init_label(tree fieldname)
{
    ...
    if (tail == 0)
        error("unknown field `%s' specified in initializer", IDENTIFIER_POINTER(fieldname));
    else if (!passed)
        error("field `%s' already initialized", IDENTIFIER_POINTER(fieldname));
    else
    {
...
static void add_pending_init(tree purpose, tree value)
{
    ...
    else
    {
        ...
        {
            p = *q;
            if (tree_int_cst_lt(DECL_FIELD_BITPOS(purpose), DECL_FIELD_BITPOS(p->purpose)))
                q = &p->left;
            else if (tree_int_cst_lt(DECL_FIELD_BITPOS(p->purpose), DECL_FIELD_BITPOS(purpose)))
                q = &p->right;
            else
                abort();
        }
...
static int pending_init_member(tree field)
{
    struct init_node *p;
    ...
    else
    {
        while (p)
        {
            if (field == p->purpose)
                return 1;
            else if (tree_int_cst_lt(DECL_FIELD_BITPOS(field), DECL_FIELD_BITPOS(p->purpose)))
                p = p->left;
            else
                p = p->right;
        }
...
static void output_init_element(tree value, tree type, tree field, int pending)
{
    ...
    if (TREE_CODE(TREE_TYPE(value)) == FUNCTION_TYPE
        || (TREE_CODE(TREE_TYPE(value)) == ARRAY_TYPE
               && !(TREE_CODE(value) == STRING_CST && TREE_CODE(type) == ARRAY_TYPE
    ...
    if (value == error_mark_node)
        ...
        || ((TREE_CODE(constructor_type) == RECORD_TYPE
                || TREE_CODE(constructor_type) == UNION_TYPE)
               && DECL_C_BIT_FIELD(field) && TREE_CODE(value) != INTEGER_CST))
        ...
        && !tree_int_cst_equal(field, constructor_unfilled_index))
    {
        if (!duplicate)
            ...
            add_pending_init(copy_node(field),
                digest_init(type, value, require_constant_value, require_constant_elements));
    }
    else if (TREE_CODE(constructor_type) == RECORD_TYPE && field != constructor_unfilled_fields)
    {
       ...
       right away since it starts at the beginning of the union.  */
        if (!duplicate)
            add_pending_init(
                field, digest_init(type, value, require_constant_value, require_constant_elements));
    }
    else
    {
       ...
       constructor_elements or to the assembler file.  */
        ...
        if (!duplicate)
        {
            ...
            else
            {
               ...
               Do this, if necessary.  */
                ...
                {
                    /* Advance to offset of this element.  */
                    if (!tree_int_cst_equal(constructor_bit_index, DECL_FIELD_BITPOS(field)))
                    {
                        /* By using unsigned arithmetic, the result will be
                       correct even in case of overflows, if BITS_PER_UNIT
                       is a power of two.  */
                        unsigned next = (TREE_INT_CST_LOW(DECL_FIELD_BITPOS(field))
                            / (unsigned)BITS_PER_UNIT);
                        unsigned here
                            = (TREE_INT_CST_LOW(constructor_bit_index) / (unsigned)BITS_PER_UNIT);

                        assemble_zeros(
                            (next - here) * (unsigned)BITS_PER_UNIT / (unsigned)BITS_PER_UNIT);
                    }
                }
                output_constant(
                    digest_init(type, value, require_constant_value, require_constant_elements),
               ...
               keep track of end position of last field.  */
                if (TREE_CODE(constructor_type) == RECORD_TYPE
                    || TREE_CODE(constructor_type) == UNION_TYPE)
                {
                    tree temp = size_binop(PLUS_EXPR, DECL_FIELD_BITPOS(field), DECL_SIZE(field));
                    TREE_INT_CST_LOW(constructor_bit_index) = TREE_INT_CST_LOW(temp);
                    TREE_INT_CST_HIGH(constructor_bit_index) = TREE_INT_CST_HIGH(temp);
                }
            }
        }
        ...
        if (TREE_CODE(constructor_type) == ARRAY_TYPE)
        {
            tree tem = size_binop(PLUS_EXPR, constructor_unfilled_index, integer_one_node);
            TREE_INT_CST_LOW(constructor_unfilled_index) = TREE_INT_CST_LOW(tem);
            TREE_INT_CST_HIGH(constructor_unfilled_index) = TREE_INT_CST_HIGH(tem);
        }
        else if (TREE_CODE(constructor_type) == RECORD_TYPE)
...
retry:
    ...
    while (elt)
    {
        if (TREE_CODE(constructor_type) == ARRAY_TYPE)
        {
            if (tree_int_cst_equal(elt->purpose, constructor_unfilled_index))
                output_init_element(
                    elt->value, TREE_TYPE(constructor_type), constructor_unfilled_index, 0);
            else if (tree_int_cst_lt(constructor_unfilled_index, elt->purpose))
            {
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (elt->parent && elt->parent->right == elt)
                        elt = elt->parent;
                    elt = elt->parent;
                    if (elt && tree_int_cst_lt(constructor_unfilled_index, elt->purpose))
                    {
                        next = elt->purpose;
                        break;
                    }
    ...
    if (constructor_incremental)
    {
        ...
        if (TREE_CODE(constructor_type) == RECORD_TYPE || TREE_CODE(constructor_type) == UNION_TYPE)
        {
            ...
            if (tail)
                ...
                filled = size_binop(CEIL_DIV_EXPR,
                    size_binop(PLUS_EXPR, DECL_FIELD_BITPOS(tail), DECL_SIZE(tail)),
                    size_int(BITS_PER_UNIT));
            else
                filled = size_int(0);

            nextpos_tree
                = size_binop(CEIL_DIV_EXPR, DECL_FIELD_BITPOS(next), size_int(BITS_PER_UNIT));

            TREE_INT_CST_HIGH(constructor_bit_index) = TREE_INT_CST_HIGH(DECL_FIELD_BITPOS(next));
            TREE_INT_CST_LOW(constructor_bit_index) = TREE_INT_CST_LOW(DECL_FIELD_BITPOS(next));
        ...
        else if (TREE_CODE(constructor_type) == ARRAY_TYPE)
        {
            filled = size_binop(
                MULT_EXPR, constructor_unfilled_index, size_in_bytes(TREE_TYPE(constructor_type)));
            nextpos_tree = size_binop(MULT_EXPR, next, size_in_bytes(TREE_TYPE(constructor_type)));
            TREE_INT_CST_LOW(constructor_unfilled_index) = TREE_INT_CST_LOW(next);
            TREE_INT_CST_HIGH(constructor_unfilled_index) = TREE_INT_CST_HIGH(next);
        }
        else
            filled = 0;
...
void process_init_element(tree value)
{
    ...
    /* Handle superfluous braces around string cst as in
       char x[] = {"foo"}; */
    if (string_flag && constructor_type && TREE_CODE(constructor_type) == ARRAY_TYPE
    ...
    while (1)
    {
        if (TREE_CODE(constructor_type) == RECORD_TYPE)
        {
            ...
            if (constructor_fields == 0)
            {
                pedwarn_init("excess elements in struct initializer%s", " after `%s'", NULL);
                break;
            }
            ...
            if (value != 0 && fieldcode == ARRAY_TYPE
                && TREE_CODE(TREE_TYPE(fieldtype)) == INTEGER_TYPE && string_flag)
                value = orig_value;
            /* Otherwise, if we have come to a subaggregate,
               and we don't have an element of its type, push into it.  */
            else if (value != 0 && !constructor_no_implicit && value != error_mark_node
                && TYPE_MAIN_VARIANT(TREE_TYPE(value)) != fieldtype
                && (fieldcode == RECORD_TYPE || fieldcode == ARRAY_TYPE || fieldcode == UNION_TYPE))
            {
                push_init_level(1);
                continue;
            }
        ...
        if (TREE_CODE(constructor_type) == ARRAY_TYPE)
        {
            ...
            /* Accept a string constant to initialize a subarray.  */
            if (value != 0 && eltcode == ARRAY_TYPE && TREE_CODE(TREE_TYPE(elttype)) == INTEGER_TYPE
                && string_flag)
                value = orig_value;
            /* Otherwise, if we have come to a subaggregate,
               and we don't have an element of its type, push into it.  */
                ...
                && (eltcode == RECORD_TYPE || eltcode == ARRAY_TYPE || eltcode == UNION_TYPE))
            {
                push_init_level(1);
                continue;
            }
            ...
            {
                if (constructor_max_index != 0
                    && tree_int_cst_lt(constructor_max_index, constructor_range_end))
                {
                    pedwarn_init("excess elements in array initializer%s", " after `%s'", NULL);
            ...
            do
            {
                ...
                tem = size_binop(PLUS_EXPR, constructor_index, integer_one_node);
                ...
                || tree_int_cst_lt(constructor_range_end, constructor_index)));
       ...
       for a scalar variable.  */
        if (constructor_fields == 0)
        {
            pedwarn_init("excess elements in scalar initializer%s", " after `%s'", NULL);
            break;
        }
    ...
    tree string, tree outputs, tree inputs, tree clobbers, int vol, const char *filename, int line)
{
    ...
    for (i = 0, tail = outputs; tail; tail = TREE_CHAIN(tail), i++)
    {
        ...
        {
            expand_expr(build_modify_expr(o[i], NOP_EXPR, TREE_VALUE(tail)), NULL_RTX, VOIDmode,
                EXPAND_NORMAL);
            free_temp_slots();
        }
       ...
       (Otherwise done by build_modify_expr.)  */
        else
        {
            ...
            if (TREE_READONLY(o[i]) || TYPE_READONLY(type)
...
void c_expand_return(tree retval)
{
    ...
    {
        tree t = convert_for_assignment(valtype, retval, "return", NULL_TREE, NULL_TREE, 0);
       ...
       we are returning the address of a local variable.  Warn if so.  */
        while (1)
        {
               ...
               don't give a warning.  */
                {
                    ...
                    while (!POINTER_TYPE_P(TREE_TYPE(op1))
                        && (TREE_CODE(op1) == NOP_EXPR || TREE_CODE(op1) == NON_LVALUE_EXPR
            ...
            case ADDR_EXPR:
                ...
                if (TREE_CODE(inner) == VAR_DECL && !DECL_EXTERNAL(inner) && !TREE_STATIC(inner)
...
tree c_expand_start_case(tree exp)
{
    ...
    else
    {
        ...
        if (warn_traditional && (type == long_integer_type_node || type == long_unsigned_type_node))
       ...
       and it's too hard to do the right thing.  */
        if (TREE_UNSIGNED(TREE_TYPE(exp)) == TREE_UNSIGNED(TREE_TYPE(index)))
            exp = index;
    }
