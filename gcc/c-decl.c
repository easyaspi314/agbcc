...
static struct binding_level clear_binding_level
    = { NULL, NULL, NULL, NULL, NULL, NULL_BINDING_LEVEL, 0, 0, 0, 0, 0, 0, NULL };
...
void finish_incomplete_decl(tree decl)
{
    if (TREE_CODE(decl) == VAR_DECL)
    {
        tree type = TREE_TYPE(decl);
        if (type != error_mark_node && TREE_CODE(type) == ARRAY_TYPE && TYPE_DOMAIN(type) == 0)
        {
...
{
    return ((current_binding_level->keep_if_subblocks && current_binding_level->blocks != 0)
        || current_binding_level->keep || current_binding_level->names != 0
        || (current_binding_level->tags != 0 && !current_binding_level->tag_transparent));
}
...
void declare_parm_level(int definition_flag ATTRIBUTE_UNUSED)
{
...
void pushlevel(int tag_transparent)
{
    ...
    *newlevel = clear_binding_level;
    newlevel->tag_transparent = (tag_transparent
        || (current_binding_level ? current_binding_level->subblocks_tag_transparent : 0));
...
tree poplevel(int keep, int reverse, int functionbody)
{
  ...
  for (link = tags; link; link = TREE_CHAIN (link))
    ...
    if (reverse)
        current_binding_level->names = decls = nreverse(current_binding_level->names);
    else
    ...
    for (decl = decls; decl; decl = TREE_CHAIN(decl))
        if (TREE_CODE(decl) == FUNCTION_DECL && !TREE_ASM_WRITTEN(decl) && DECL_INITIAL(decl) != 0
            && TREE_ADDRESSABLE(decl))
        {
            ...
            if (DECL_ABSTRACT_ORIGIN(decl) != 0 && DECL_ABSTRACT_ORIGIN(decl) != decl)
    ...
    else if (keep || functionbody || (current_binding_level->keep_if_subblocks && subblocks != 0))
       ...
       and add them to BLOCK_VARS.  */
        ...
        for (link = named_labels; link; link = TREE_CHAIN(link))
        {
            ...
            if (DECL_INITIAL(label) == 0)
            {
                ...
                define_label(input_filename, lineno, DECL_NAME(label));
            }
    ...
    /* If we did not make a block for the level just exited,
       ...
       of something else.  */
        ...
        current_binding_level->blocks = chainon(current_binding_level->blocks, subblocks);
...
void pop_label_level(void)
{
    ...
    for (link = named_labels, prev = 0; link;)
    {
        if (C_DECLARED_LABEL_FLAG(TREE_VALUE(link)))
        {
            ...
            {
                error_with_decl(TREE_VALUE(link), "label `%s' used but not defined");
                /* Avoid crashing later.  */
                define_label(input_filename, lineno, DECL_NAME(TREE_VALUE(link)));
            }
            else if (warn_unused && !TREE_USED(TREE_VALUE(link)))
                warning_with_decl(TREE_VALUE(link), "label `%s' defined but not used");
    ...
    for (link = shadowed_labels; link; link = TREE_CHAIN(link))
        if (DECL_NAME(TREE_VALUE(link)) != 0)
            IDENTIFIER_LABEL_VALUE(DECL_NAME(TREE_VALUE(link))) = TREE_VALUE(link);
    ...
    tree newdecl, tree olddecl, int different_binding_level)
{
    ...
    if (TREE_CODE(newtype) == ERROR_MARK || TREE_CODE(oldtype) == ERROR_MARK)
        types_match = 0;
            ...
            && (DECL_BUILT_IN(olddecl) || DECL_BUILT_IN_NONANSI(olddecl)))
        {
            ...
            /* Likewise, if the built-in is not ansi, then programs can
               override it even globally without an error.  */
                ...
                warning_with_decl(newdecl, "library function `%s' declared as non-function");

            else if (DECL_BUILT_IN_NONANSI(olddecl))
                warning_with_decl(newdecl, "built-in function `%s' declared as non-function");
            else
                warning_with_decl(newdecl, "built-in function `%s' declared as non-function");
        }
        else
    ...
    if (types_match && TREE_CODE(newdecl) == PARM_DECL && TREE_ASM_WRITTEN(olddecl)
    ...
    if (flag_traditional && TREE_CODE(newdecl) == FUNCTION_DECL
        && IDENTIFIER_IMPLICIT_DECL(DECL_NAME(newdecl)) == olddecl && DECL_INITIAL(olddecl) == 0)
        ...
        else if (!types_match)
        {
            ...
            if (TYPE_MODE(oldreturntype) == TYPE_MODE(newreturntype))
            {
               ...
               the return type of olddecl's function type.  */
                tree trytype = build_function_type(newreturntype, TYPE_ARG_TYPES(oldtype));
            ...
            /* Accept harmless mismatch in first argument type also.
               This is for ffs.  */
            if (TYPE_ARG_TYPES(TREE_TYPE(newdecl)) != 0 && TYPE_ARG_TYPES(oldtype) != 0
                       ...
                       == TYPE_MODE(TREE_VALUE(TYPE_ARG_TYPES(oldtype)))))
            {
                /* Function types may be shared, so we can't just modify
               the return type of olddecl's function type.  */
                tree trytype = build_function_type(TREE_TYPE(oldtype),
                    tree_cons(NULL_TREE,
    ...
    else if (!types_match && TREE_CODE(olddecl) == FUNCTION_DECL
        && TREE_CODE(newdecl) == FUNCTION_DECL && TREE_CODE(TREE_TYPE(oldtype)) == POINTER_TYPE
        && TREE_CODE(TREE_TYPE(newtype)) == POINTER_TYPE
        && (DECL_IN_SYSTEM_HEADER(olddecl) || DECL_IN_SYSTEM_HEADER(newdecl))
        && ((TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(newtype))) == void_type_node
                && TYPE_ARG_TYPES(oldtype) == 0 && self_promoting_args_p(TYPE_ARG_TYPES(newtype))
                && TREE_TYPE(TREE_TYPE(oldtype)) == char_type_node)
               || (TREE_TYPE(TREE_TYPE(newtype)) == char_type_node && TYPE_ARG_TYPES(newtype) == 0
               ...
               && TYPE_ARG_TYPES(newtype) == 0))
    {
       ...
       involving an empty arglist vs a nonempty one.  */
        if (TREE_CODE(olddecl) == FUNCTION_DECL && comptypes(TREE_TYPE(oldtype), TREE_TYPE(newtype))
                   ...
                   || (TYPE_ARG_TYPES(newtype) == 0 && DECL_INITIAL(newdecl) == 0)))
        {
            ...
            for (; t; t = TREE_CHAIN(t))
            {
                ...
                if (TYPE_MAIN_VARIANT(type) == float_type_node || C_PROMOTING_INTEGER_TYPE_P(type))
                {
    ...
    else
    {
        ...
        else if (TREE_CODE(olddecl) == FUNCTION_DECL && DECL_INITIAL(olddecl) != 0
            ...
            && TYPE_ACTUAL_ARG_TYPES(oldtype) != 0)
        {
            ...
            if (errmsg)
            {
                ...
                error_with_decl(olddecl, "doesn't match non-prototype definition here");
            }
            else
            {
        ...
        else
        {
            ...
            if (TREE_CODE(olddecl) == FUNCTION_DECL && !DECL_INLINE(olddecl) && DECL_INLINE(newdecl)
                && TREE_USED(olddecl))
                warning_with_decl(newdecl, "`%s' declared inline after being called");
            if (TREE_CODE(olddecl) == FUNCTION_DECL && !DECL_INLINE(olddecl) && DECL_INLINE(newdecl)
                && DECL_INITIAL(olddecl) != 0)
                warning_with_decl(newdecl, "`%s' declared inline after its definition");

            /* If pedantic, warn when static declaration follows a non-static
               declaration.  Otherwise, do so only for functions.  */
            if ((pedantic || TREE_CODE(olddecl) == FUNCTION_DECL) && TREE_PUBLIC(olddecl)
                && !TREE_PUBLIC(newdecl))
                warning_with_decl(newdecl, "static declaration for `%s' follows non-static");

            /* Warn when const declaration follows a non-const
               declaration, but not for functions.  */
            if (TREE_CODE(olddecl) != FUNCTION_DECL && !TREE_READONLY(olddecl)
    ...
    if (errmsg == 0 && warn_redundant_decls
    ...
    if (types_match)
    {
       ...
       one.  */
        ...
        {
            if (different_binding_level)
                TREE_TYPE(newdecl) = build_type_attribute_variant(
                    newtype, merge_attributes(TYPE_ATTRIBUTES(newtype), TYPE_ATTRIBUTES(oldtype)));
            else
                TREE_TYPE(newdecl) = TREE_TYPE(olddecl) = common_type(newtype, oldtype);
        }
        ...
        if (oldtype != TREE_TYPE(newdecl))
        {
            ...
            if (TREE_CODE(newdecl) != FUNCTION_DECL && TREE_CODE(newdecl) != TYPE_DECL
       ...
       information so that meaningful diagnostics can be given.  */
        if (DECL_INITIAL(newdecl) == 0 && DECL_INITIAL(olddecl) != 0 && !different_binding_level)
        {
       ...
       are assigned.  */
        ...
        if (TREE_CODE(newdecl) == FUNCTION_DECL)
        {
            ...
            DECL_NO_CHECK_MEMORY_USAGE(newdecl) |= DECL_NO_CHECK_MEMORY_USAGE(olddecl);
        }
...
tree pushdecl(tree x)
{
    ...
    if (TREE_CODE(x) == FUNCTION_DECL && DECL_INITIAL(x) == 0 && DECL_EXTERNAL(x))
        DECL_CONTEXT(x) = 0;
    ...
    if (name)
    {
       ...
       since traditional compilers don't complain.  */
            ...
            && IDENTIFIER_IMPLICIT_DECL(name) != 0)
        {
            ...
            pedwarn_with_file_and_line(DECL_SOURCE_FILE(IDENTIFIER_IMPLICIT_DECL(name)),
       ...
       standard built-in type.
        ...
        if (TREE_CODE(x) == TYPE_DECL)
        {
            ...
            else if (TREE_TYPE(x) != error_mark_node && DECL_ORIGINAL_TYPE(x) == NULL_TREE)
            {
                tree tt = TREE_TYPE(x);
       ...
       Avoid duplicate warnings where they are used.  */
        if (TREE_PUBLIC(x) && !DECL_INLINE(x))
        {
            ...
            if (decl
                ...
                && !DECL_BUILT_IN(decl))
            {
                pedwarn_with_decl(x, "type mismatch with previous external decl");
       ...
       make sure they are compatible.  */
        ...
        if (IDENTIFIER_IMPLICIT_DECL(name) != 0 && IDENTIFIER_GLOBAL_VALUE(name) == 0
            && TREE_CODE(x) == FUNCTION_DECL
            && !comptypes(TREE_TYPE(x), TREE_TYPE(IDENTIFIER_IMPLICIT_DECL(name))))
        {
            warning_with_decl(x, "type mismatch with previous implicit declaration");
            warning_with_decl(
                IDENTIFIER_IMPLICIT_DECL(name), "previous implicit declaration of `%s'");
        }

        /* In PCC-compatibility mode, extern decls of vars with no current decl
       take effect at top level no matter where they are.  */
        if (flag_traditional && DECL_EXTERNAL(x) && lookup_name(name) == 0)
        {
            ...
            while (type)
            {
                ...
                else if (TREE_CODE(type) == FUNCTION_TYPE && TYPE_ARG_TYPES(type) != 0)
       ...
       Install the new declaration and return it.  */
        if (b == global_binding_level)
        {
            ...
            if (IDENTIFIER_IMPLICIT_DECL(name) && TREE_USED(IDENTIFIER_IMPLICIT_DECL(name)))
                TREE_USED(x) = 1, TREE_USED(name) = 1;

            /* Don't forget if its address was taken in that way.  */
            if (IDENTIFIER_IMPLICIT_DECL(name) && TREE_ADDRESSABLE(IDENTIFIER_IMPLICIT_DECL(name)))
            ...
            if (IDENTIFIER_IMPLICIT_DECL(name) != 0
                ...
                && !(TREE_CODE(x) == FUNCTION_DECL
                       && (TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(x))) == integer_type_node)))
                pedwarn("`%s' was previously implicitly declared to return `int'",
                    IDENTIFIER_POINTER(name));

            /* If this decl is `static' and an `extern' was seen previously,
               that is erroneous.  */
            if (TREE_PUBLIC(name) && !TREE_PUBLIC(x) && !DECL_EXTERNAL(x))
            {
               ...
               declaration inside a lexical block.  */
                ...
                else if (IDENTIFIER_IMPLICIT_DECL(name))
                {
                    ...
                    pedwarn(
                        "`%s' was declared `extern' and later `static'", IDENTIFIER_POINTER(name));
            }
        }
        else
        {
            ...
            if (oldlocal == 0 && DECL_EXTERNAL(x) && !DECL_INLINE(x) && oldglobal != 0
                && TREE_CODE(x) == FUNCTION_DECL && TREE_CODE(oldglobal) == FUNCTION_DECL)
            {
                /* We have one.  Their types must agree.  */
                if (!comptypes(TREE_TYPE(x), TREE_TYPE(IDENTIFIER_GLOBAL_VALUE(name))))
                ...
                else
                {
                    ...
                    if (DECL_INLINE(oldglobal))
                    {
                        ...
                        DECL_INITIAL(x)
                            = (current_function_decl == oldglobal ? 0 : DECL_INITIAL(oldglobal));
                    ...
                    if (TYPE_ARG_TYPES(TREE_TYPE(oldglobal)) != 0 && DECL_INITIAL(oldglobal)
	  ...
	  if (oldlocal == 0 && oldglobal != 0
	    ...
	    warning ("`%s' locally external but globally static",
            ...
            if (oldlocal == 0 && DECL_EXTERNAL(x) && TREE_PUBLIC(x))
            {
                if (oldglobal == 0)
                    TREE_PUBLIC(name) = 1;
                ...
                && chain_member(oldlocal, current_binding_level->level_chain->names))
            {
                if (TREE_CODE(oldlocal) == PARM_DECL)
                    pedwarn("declaration of `%s' shadows a parameter", IDENTIFIER_POINTER(name));
                else
...
tree implicitly_declare(tree functionid)
{
    ...
    {
        if (mesg_implicit_function_declaration == 2)
            error("implicit declaration of function `%s'", IDENTIFIER_POINTER(functionid));
        else
            warning("implicit declaration of function `%s'", IDENTIFIER_POINTER(functionid));
    }
    else if (warn_traditional && traditional_warning)
        warning(
            "function `%s' was previously declared within a block", IDENTIFIER_POINTER(functionid));
...
static const char *redeclaration_error_message(tree newdecl, tree olddecl)
{
    if (TREE_CODE(newdecl) == TYPE_DECL)
    {
       ...
       code, because it doesn't have type qualifiers.  */
        if (flag_traditional && TYPE_MAIN_VARIANT(TREE_TYPE(olddecl)) == TREE_TYPE(newdecl))
            return 0;
    ...
    else if (current_binding_level->parm_flag && TREE_ASM_WRITTEN(olddecl)
        && !TREE_ASM_WRITTEN(newdecl))
        return 0;
    else
    {
...
tree lookup_label(tree id)
{
    ...
    {
        error("label %s referenced outside of any function", IDENTIFIER_POINTER(id));
        return 0;
    }
    ...
    if (decl != 0)
    {
        ...
        if (DECL_CONTEXT(decl) != current_function_decl && !C_DECLARED_LABEL_FLAG(decl))
       ...
       at this label scope */
        for (dup = named_labels; dup; dup = TREE_CHAIN(dup))
            if (TREE_VALUE(dup) == decl)
            {
                ...
                error_with_decl(TREE_VALUE(dup), "this is a previous declaration");
...
void init_decl_processing(void)
{
    ...
    pushdecl(build_decl(TYPE_DECL, ridpointers[(int)RID_INT], integer_type_node));
    ...
    pushdecl(build_decl(TYPE_DECL, get_identifier("long int"), long_integer_type_node));

    unsigned_type_node = make_unsigned_type(INT_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("unsigned int"), unsigned_type_node));

    long_unsigned_type_node = make_unsigned_type(LONG_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("long unsigned int"), long_unsigned_type_node));

    long_long_integer_type_node = make_signed_type(LONG_LONG_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("long long int"), long_long_integer_type_node));

    long_long_unsigned_type_node = make_unsigned_type(LONG_LONG_TYPE_SIZE);
    pushdecl(build_decl(
        TYPE_DECL, get_identifier("long long unsigned int"), long_long_unsigned_type_node));

    short_integer_type_node = make_signed_type(SHORT_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("short int"), short_integer_type_node));

    short_unsigned_type_node = make_unsigned_type(SHORT_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("short unsigned int"), short_unsigned_type_node));

    /* `unsigned long' is the standard type for sizeof.
       Traditionally, use a signed type.
       Note that stddef.h uses `unsigned long',
       and this must agree, even if long and int are the same size.  */
    set_sizetype(TREE_TYPE(IDENTIFIER_GLOBAL_VALUE(get_identifier(SIZE_TYPE))));
    ...
    pushdecl(build_decl(TYPE_DECL, get_identifier("signed char"), signed_char_type_node));

    unsigned_char_type_node = make_unsigned_type(CHAR_TYPE_SIZE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("unsigned char"), unsigned_char_type_node));
    ...
    pushdecl(build_decl(TYPE_DECL, ridpointers[(int)RID_FLOAT], float_type_node));
    ...
    pushdecl(build_decl(TYPE_DECL, ridpointers[(int)RID_DOUBLE], double_type_node));
    layout_type(double_type_node);

    long_double_type_node = make_node(REAL_TYPE);
    TYPE_PRECISION(long_double_type_node) = LONG_DOUBLE_TYPE_SIZE;
    pushdecl(build_decl(TYPE_DECL, get_identifier("long double"), long_double_type_node));
    layout_type(long_double_type_node);

    complex_integer_type_node = make_node(COMPLEX_TYPE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("complex int"), complex_integer_type_node));
    TREE_TYPE(complex_integer_type_node) = integer_type_node;
    layout_type(complex_integer_type_node);

    complex_float_type_node = make_node(COMPLEX_TYPE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("complex float"), complex_float_type_node));
    TREE_TYPE(complex_float_type_node) = float_type_node;
    layout_type(complex_float_type_node);

    complex_double_type_node = make_node(COMPLEX_TYPE);
    pushdecl(build_decl(TYPE_DECL, get_identifier("complex double"), complex_double_type_node));
    ...
    wchar_type_node = TREE_TYPE(IDENTIFIER_GLOBAL_VALUE(get_identifier(WCHAR_TYPE)));
    ...
    pushdecl(build_decl(TYPE_DECL, ridpointers[(int)RID_VOID], void_type_node));
    ...
    int_array_type_node = build_array_type(integer_type_node, array_domain_type);
    /* This is for wide string constants.  */
    wchar_array_type_node = build_array_type(wchar_type_node, array_domain_type);
    ...
    float_ftype_float
        = build_function_type(float_type_node, tree_cons(NULL_TREE, float_type_node, endlink));

    double_ftype_double
        = build_function_type(double_type_node, tree_cons(NULL_TREE, double_type_node, endlink));

    ldouble_ftype_ldouble = build_function_type(
        long_double_type_node, tree_cons(NULL_TREE, long_double_type_node, endlink));

    double_ftype_double_double = build_function_type(double_type_node,
        tree_cons(NULL_TREE, double_type_node, tree_cons(NULL_TREE, double_type_node, endlink)));

    int_ftype_int
        = build_function_type(integer_type_node, tree_cons(NULL_TREE, integer_type_node, endlink));

    long_ftype_long = build_function_type(
        long_integer_type_node, tree_cons(NULL_TREE, long_integer_type_node, endlink));

    void_ftype_ptr_ptr_int = build_function_type(void_type_node,
        tree_cons(NULL_TREE, ptr_type_node,
            tree_cons(NULL_TREE, ptr_type_node, tree_cons(NULL_TREE, integer_type_node, endlink))));

    int_ftype_cptr_cptr_sizet = build_function_type(integer_type_node,
        tree_cons(NULL_TREE, const_ptr_type_node,
            tree_cons(NULL_TREE, const_ptr_type_node, tree_cons(NULL_TREE, sizetype, endlink))));

    void_ftype_ptr_int_int = build_function_type(void_type_node,
        tree_cons(NULL_TREE, ptr_type_node,
            tree_cons(
                NULL_TREE, integer_type_node, tree_cons(NULL_TREE, integer_type_node, endlink))));

    string_ftype_ptr_ptr /* strcpy prototype */
        = build_function_type(string_type_node,
            tree_cons(NULL_TREE, string_type_node,
                tree_cons(NULL_TREE, const_string_type_node, endlink)));
    ...
    traditional_ptr_type_node = (flag_traditional ? string_type_node : ptr_type_node);

    memcpy_ftype /* memcpy prototype */
        = build_function_type(traditional_ptr_type_node,
            tree_cons(NULL_TREE, ptr_type_node,
                tree_cons(
                    NULL_TREE, const_ptr_type_node, tree_cons(NULL_TREE, sizetype, endlink))));

    memset_ftype /* memset prototype */
        = build_function_type(traditional_ptr_type_node,
            tree_cons(NULL_TREE, ptr_type_node,
                tree_cons(NULL_TREE, integer_type_node, tree_cons(NULL_TREE, sizetype, endlink))));

    ptr_ftype_void = build_function_type(ptr_type_node, endlink);
    ptr_ftype_ptr
        = build_function_type(ptr_type_node, tree_cons(NULL_TREE, ptr_type_node, endlink));

    builtin_function("__builtin_constant_p", default_function_type, BUILT_IN_CONSTANT_P, NULL);
    ...
    builtin_function("__builtin_aggregate_incoming_address",
        build_function_type(ptr_type_node, NULL_TREE), BUILT_IN_AGGREGATE_INCOMING_ADDRESS, NULL);

    /* Hooks for the DWARF 2 __throw routine.  */
    builtin_function("__builtin_unwind_init", build_function_type(void_type_node, endlink),
        BUILT_IN_UNWIND_INIT, NULL);
    builtin_function("__builtin_dwarf_cfa", ptr_ftype_void, BUILT_IN_DWARF_CFA, NULL);
    builtin_function("__builtin_dwarf_fp_regnum", build_function_type(unsigned_type_node, endlink),
        BUILT_IN_DWARF_FP_REGNUM, NULL);
    builtin_function("__builtin_dwarf_reg_size", int_ftype_int, BUILT_IN_DWARF_REG_SIZE, NULL);
    builtin_function("__builtin_frob_return_addr", ptr_ftype_ptr, BUILT_IN_FROB_RETURN_ADDR, NULL);
    builtin_function(
        "__builtin_extract_return_addr", ptr_ftype_ptr, BUILT_IN_EXTRACT_RETURN_ADDR, NULL);
    builtin_function("__builtin_eh_return",
    ...
    builtin_function("__builtin_alloca",
        build_function_type(ptr_type_node, tree_cons(NULL_TREE, sizetype, endlink)),
    ...
    if (!flag_no_builtin && !flag_no_nonansi_builtin)
    {
        ...
        /* Suppress error if redefined as a non-function.  */
        DECL_BUILT_IN_NONANSI(temp) = 1;
        temp = builtin_function("_exit", void_ftype_any, NOT_BUILT_IN, NULL);
    ...
    builtin_function("__builtin_fabsf", float_ftype_float, BUILT_IN_FABS, NULL);
    builtin_function("__builtin_fabs", double_ftype_double, BUILT_IN_FABS, NULL);
    builtin_function("__builtin_fabsl", ldouble_ftype_ldouble, BUILT_IN_FABS, NULL);
    builtin_function("__builtin_labs", long_ftype_long, BUILT_IN_LABS, NULL);
    builtin_function("__builtin_saveregs", build_function_type(ptr_type_node, NULL_TREE),
...
/* EXPAND_BUILTIN_VARARGS is obsolete.  */
  ...
  builtin_function ("__builtin_varargs",
...
#endif
    builtin_function(
        "__builtin_classify_type", default_function_type, BUILT_IN_CLASSIFY_TYPE, NULL);
    builtin_function("__builtin_next_arg", build_function_type(ptr_type_node, NULL_TREE),
    ...
    builtin_function("__builtin_apply_args", build_function_type(ptr_type_node, NULL_TREE),
        BUILT_IN_APPLY_ARGS, NULL);

    temp = tree_cons(NULL_TREE,
        build_pointer_type(build_function_type(void_type_node, NULL_TREE)),
        tree_cons(NULL_TREE, ptr_type_node, tree_cons(NULL_TREE, sizetype, endlink)));
    builtin_function(
        "__builtin_apply", build_function_type(ptr_type_node, temp), BUILT_IN_APPLY, NULL);
    builtin_function("__builtin_return",
        build_function_type(void_type_node, tree_cons(NULL_TREE, ptr_type_node, endlink)),
    ...
    builtin_function("__builtin_memcpy", memcpy_ftype, BUILT_IN_MEMCPY, "memcpy");
    builtin_function("__builtin_memcmp", int_ftype_cptr_cptr_sizet, BUILT_IN_MEMCMP, "memcmp");
    builtin_function("__builtin_memset", memset_ftype, BUILT_IN_MEMSET, "memset");
    builtin_function("__builtin_strcmp", int_ftype_string_string, BUILT_IN_STRCMP, "strcmp");
    builtin_function("__builtin_strcpy", string_ftype_ptr_ptr, BUILT_IN_STRCPY, "strcpy");
    builtin_function("__builtin_strlen", strlen_ftype, BUILT_IN_STRLEN, "strlen");
    builtin_function("__builtin_sqrtf", float_ftype_float, BUILT_IN_FSQRT, "sqrtf");
    builtin_function("__builtin_fsqrt", double_ftype_double, BUILT_IN_FSQRT, "sqrt");
    builtin_function("__builtin_sqrtl", ldouble_ftype_ldouble, BUILT_IN_FSQRT, "sqrtl");
    builtin_function("__builtin_sinf", float_ftype_float, BUILT_IN_SIN, "sinf");
    builtin_function("__builtin_sin", double_ftype_double, BUILT_IN_SIN, "sin");
    builtin_function("__builtin_sinl", ldouble_ftype_ldouble, BUILT_IN_SIN, "sinl");
    builtin_function("__builtin_cosf", float_ftype_float, BUILT_IN_COS, "cosf");
    builtin_function("__builtin_cos", double_ftype_double, BUILT_IN_COS, "cos");
    builtin_function("__builtin_cosl", ldouble_ftype_ldouble, BUILT_IN_COS, "cosl");
    builtin_function("__builtin_setjmp",
        build_function_type(integer_type_node, tree_cons(NULL_TREE, ptr_type_node, endlink)),
        BUILT_IN_SETJMP, NULL);
    builtin_function("__builtin_longjmp",
        build_function_type(void_type_node,
            tree_cons(NULL, ptr_type_node, tree_cons(NULL_TREE, integer_type_node, endlink))),
        BUILT_IN_LONGJMP, NULL);
    builtin_function(
        "__builtin_trap", build_function_type(void_type_node, endlink), BUILT_IN_TRAP, NULL);
    ...
    if (!flag_no_builtin)
    {
        ...
        builtin_function("fabsl", ldouble_ftype_ldouble, BUILT_IN_FABS, NULL);
        builtin_function("labs", long_ftype_long, BUILT_IN_LABS, NULL);
        builtin_function("memcpy", memcpy_ftype, BUILT_IN_MEMCPY, NULL);
        builtin_function("memcmp", int_ftype_cptr_cptr_sizet, BUILT_IN_MEMCMP, NULL);
        builtin_function("memset", memset_ftype, BUILT_IN_MEMSET, NULL);
        builtin_function("strcmp", int_ftype_string_string, BUILT_IN_STRCMP, NULL);
        builtin_function("strcpy", string_ftype_ptr_ptr, BUILT_IN_STRCPY, NULL);
        ...
        builtin_function("sqrtl", ldouble_ftype_ldouble, BUILT_IN_FSQRT, NULL);
       ...
       from certain code which isn't valid in ANSI but which exists.  */
        temp = builtin_function("abort", void_ftype_any, NOT_BUILT_IN, NULL);
        TREE_THIS_VOLATILE(temp) = 1;
        TREE_SIDE_EFFECTS(temp) = 1;
...
/* warned 1 => we have done a pedwarn.  2 => we have done a warning, but
no pedwarn.  */
{
    int found_tag = 0;
    tree link;
...
tree groktypename(tree type_name)
{
    ...
    tree declarator, tree declspecs, int initialized, tree attributes, tree prefix_attributes)
{
    ...
    if (initialized)
        ...
        case TYPE_DECL:
            ...
            {
                error("typedef `%s' is initialized", IDENTIFIER_POINTER(DECL_NAME(decl)));
                initialized = 0;
            }
            break;
        ...
        case PARM_DECL:
            ...
            error("parameter `%s' is initialized", IDENTIFIER_POINTER(DECL_NAME(decl)));
            initialized = 0;
            break;
        ...
        && DECL_RTL(tem) == 0)
    {
        if (TYPE_SIZE(TREE_TYPE(tem)) != 0)
            expand_decl(tem);
        else if (TREE_CODE(TREE_TYPE(tem)) == ARRAY_TYPE && DECL_INITIAL(tem) != 0)
            expand_decl(tem);
    }
...
void finish_decl(tree decl, tree init, tree asmspec_tree)
{
  ...
  if (current_binding_level == global_binding_level && temporary)
    ...
    if (TREE_CODE(type) == ARRAY_TYPE && TYPE_DOMAIN(type) == 0 && TREE_CODE(decl) != TYPE_DECL)
    {
        int do_default = (TREE_STATIC(decl)
                ...
                : !DECL_EXTERNAL(decl));
        int failure = complete_array_type(type, DECL_INITIAL(decl), do_default);
    ...
    if (TREE_CODE(decl) == VAR_DECL)
    {
        ...
        if ((DECL_EXTERNAL(decl) || TREE_STATIC(decl)) && DECL_SIZE(decl) != 0)
        {
    ...
    {
        if ((flag_traditional || TREE_PERMANENT(decl)) && allocation_temporary_p())
        {
            push_obstacks_nochange();
            end_temporary_allocation();
            rest_of_decl_compilation(
                decl, asmspec, (DECL_CONTEXT(decl) == 0 || TREE_ASM_WRITTEN(decl)), 0);
            pop_obstacks();
        }
        else
        {
            rest_of_decl_compilation(decl, asmspec, DECL_CONTEXT(decl) == 0, 0);
        }
...
tree maybe_build_cleanup(tree decl ATTRIBUTE_UNUSED)
{
...
void push_parm_decl(tree parm)
{
    ...
    decl
        = grokdeclarator(TREE_VALUE(TREE_PURPOSE(parm)), TREE_PURPOSE(TREE_PURPOSE(parm)), PARM, 0);
    decl_attributes(decl, TREE_VALUE(TREE_VALUE(parm)), TREE_PURPOSE(TREE_VALUE(parm)));
...
int complete_array_type(tree type, tree initial_value, int do_default)
{
    ...
    if (initial_value)
    {
       ...
       one less than the size.  */
        ...
        {
            int eltsize = int_size_in_bytes(TREE_TYPE(TREE_TYPE(initial_value)));
...
static tree grokdeclarator(
    tree declarator, tree declspecs, enum decl_context decl_context, int initialized)
{
    int specbits = 0;
    tree spec;
    tree type = NULL_TREE;
    ...
    if (decl_context == NORMAL && !funcdef_flag && current_binding_level->parm_flag)
        decl_context = PARM;
    ...
    {
        if ((!(specbits
                & ((1 << (int)RID_LONG) | (1 << (int)RID_SHORT) | (1 << (int)RID_SIGNED)
                      | (1 << (int)RID_UNSIGNED))))
    ...
    if ((specbits & 1 << (int)RID_LONG) && !longlong && TYPE_MAIN_VARIANT(type) == double_type_node)
    {
    ...
    if (specbits
        & ((1 << (int)RID_LONG) | (1 << (int)RID_SHORT) | (1 << (int)RID_UNSIGNED)
              | (1 << (int)RID_SIGNED)))
    {
        int ok = 0;

        if ((specbits & 1 << (int)RID_LONG) && (specbits & 1 << (int)RID_SHORT))
        ...
        if (!ok)
        {
            specbits &= ~((1 << (int)RID_LONG) | (1 << (int)RID_SHORT) | (1 << (int)RID_UNSIGNED)
                | (1 << (int)RID_SIGNED));
            longlong = 0;
        }
    }

    if ((specbits & (1 << (int)RID_COMPLEX)) && TREE_CODE(type) != INTEGER_TYPE
        && TREE_CODE(type) != REAL_TYPE)
    {
    ...
    if (specbits & 1 << (int)RID_UNSIGNED
        ...
        || (bitfield && !flag_signed_bitfields
               ...
               && TREE_CODE(type) != ENUMERAL_TYPE && !(specbits & 1 << (int)RID_SIGNED)))
    {
        if (longlong)
    ...
    else if ((specbits & 1 << (int)RID_SIGNED) && type == char_type_node)
    ...
    type_quals = ((constp ? TYPE_QUAL_CONST : 0) | (restrictp ? TYPE_QUAL_RESTRICT : 0)
...

    {
                   ...
                   & ((1 << (int)RID_REGISTER) | (1 << (int)RID_AUTO) | (1 << (int)RID_TYPEDEF))))
        {
            ...
            specbits
                &= ~((1 << (int)RID_TYPEDEF) | (1 << (int)RID_REGISTER) | (1 << (int)RID_AUTO));
        }
        else if (decl_context != NORMAL && nclasses > 0)
        {
            ...
            else
            {
                ...
                specbits &= ~((1 << (int)RID_TYPEDEF) | (1 << (int)RID_REGISTER)
                    | (1 << (int)RID_AUTO) | (1 << (int)RID_STATIC) | (1 << (int)RID_EXTERN));
            }
        }
        ...
        else if (current_binding_level == global_binding_level && specbits & (1 << (int)RID_AUTO))
            error("top-level declaration of `%s' specifies `auto'", name);
        else if ((specbits & 1 << (int)RID_ITERATOR) && TREE_CODE(declarator) != IDENTIFIER_NODE)
        {
            error("iterator `%s' has derived type", name);
            type = error_mark_node;
        }
        else if ((specbits & 1 << (int)RID_ITERATOR) && TREE_CODE(type) != INTEGER_TYPE)
        {
        ...
        if (TREE_CODE(declarator) == ARRAY_REF)
        {
            ...
            if (size)
            {
                ...
                else
                {
                    ...
                    if (pedantic)
                    {
                        if (TREE_CONSTANT(size))
                            pedwarn(
                                "ANSI C forbids array `%s' whose size can't be evaluated", name);
                        else
               ...
               the computations will be done in the proper mode.  */
                itype = fold(build(MINUS_EXPR, index_type, convert(index_type, size),
     ...
     callers would ignore them, and pass arguments wrong.  */
	  ...
	  type = build_function_type (type,
        ...
        else if (TREE_CODE(declarator) == INDIRECT_REF)
        {
            ...
            if (pedantic && TREE_CODE(type) == FUNCTION_TYPE && type_quals)
    ...
    if (TREE_CODE(type) == ARRAY_TYPE && TYPE_SIZE(type) && TREE_OVERFLOW(TYPE_SIZE(type)))
        error("size of array `%s' is too large", name);
    ...
    if (type != 0 && typedef_type != 0 && TYPE_MAIN_VARIANT(type) == TYPE_MAIN_VARIANT(typedef_type)
...

    {
        ...
        if (decl_context == PARM)
        {
            ...
            main_type = (type == error_mark_node ? error_mark_node : TYPE_MAIN_VARIANT(type));
            ...
            else if (TREE_CODE(main_type) != ERROR_MARK && C_PROMOTING_INTEGER_TYPE_P(main_type))
            {
        ...
        else if (decl_context == FIELD)
        {
            ...
            if (TREE_CODE(type) == ARRAY_TYPE && type_quals)
            {
                type = build_array_type(
                    c_build_qualified_type(TREE_TYPE(type), type_quals), TYPE_DOMAIN(type));
        ...
        else if (TREE_CODE(type) == FUNCTION_TYPE)
        {
            ...
            if (current_binding_level != global_binding_level
                && (specbits & ((1 << (int)RID_STATIC) | (1 << (int)RID_INLINE))) && pedantic)
                pedwarn("invalid storage class for function `%s'", name);
            ...
            if (pedantic && TYPE_MAIN_VARIANT(TREE_TYPE(TREE_TYPE(decl))) == void_type_node
                && TYPE_QUALS(TREE_TYPE(TREE_TYPE(decl))) && !DECL_IN_SYSTEM_HEADER(decl))
                pedwarn("ANSI C forbids qualified void function return type");

            /* GNU C interprets a `volatile void' return type to indicate
               that the function does not return.  */
            if ((type_quals & TYPE_QUAL_VOLATILE) && TREE_TYPE(TREE_TYPE(decl)) != void_type_node)
                warning("`noreturn' function returns non-void value");

            if (extern_ref)
                DECL_EXTERNAL(decl) = 1;
            /* Record absence of global scope for `static' or `auto'.  */
            TREE_PUBLIC(decl) = !(specbits & ((1 << (int)RID_STATIC) | (1 << (int)RID_AUTO)));
	    ...
	    type_quals = TYPE_UNQUALIFIED;
            ...
            {
                TREE_PUBLIC(decl)
                    = !(specbits & ((1 << (int)RID_STATIC) | (1 << (int)RID_REGISTER)));
...
static tree grokparms(tree parms_info, int funcdef_flag)
{
    ...
    if (warn_strict_prototypes && first_parm == 0 && !funcdef_flag && !in_system_header)
        warning("function declaration isn't a prototype");

    if (first_parm != 0 && TREE_CODE(TREE_VALUE(first_parm)) == IDENTIFIER_NODE)
    {
        if (!funcdef_flag)
    ...
    else
    {
      ...
      if (funcdef_flag)
#endif
        for (parm = last_function_parms, typelt = first_parm; parm; parm = TREE_CHAIN(parm))
     ...
     which uses a more accurate criterion for what to warn about.  */
        ...
        if (first_parm && !TREE_PERMANENT(first_parm))
        {
            ...
            for (typelt = first_parm; typelt; typelt = TREE_CHAIN(typelt))
                result = saveable_tree_cons(NULL_TREE, TREE_VALUE(typelt), result);
...
tree get_parm_info(int void_at_end)
{
    ...
    if (void_at_end && parms != 0 && TREE_CHAIN(parms) == 0
        && TYPE_MAIN_VARIANT(TREE_TYPE(parms)) == void_type_node && DECL_NAME(parms) == 0)
    {
        parms = NULL_TREE;
        storedecls(NULL_TREE);
        return saveable_tree_cons(
            NULL_TREE, NULL_TREE, saveable_tree_cons(NULL_TREE, void_type_node, NULL_TREE));
    }
    ...
    new_parms = chainon(order ? nreverse(TREE_VALUE(order)) : 0, new_parms);
    ...
    if (void_at_end)
        return saveable_tree_cons(
            new_parms, tags, nreverse(saveable_tree_cons(NULL_TREE, void_type_node, types)));
       ...
       So don't warn for that.  */
        ...
        if (TREE_PURPOSE(elt) != 0)
            warning("`%s %s' declared inside parameter list",
                (code == RECORD_TYPE ? "struct" : code == UNION_TYPE ? "union" : "enum"),
...
tree start_struct(enum tree_code code, tree name)
{
    ...
    if (ref && TREE_CODE(ref) == code)
    {
        ...
        if (TYPE_FIELDS(ref))
            error(
                (code == UNION_TYPE ? "redefinition of `union %s'" : "redefinition of `struct %s'"),
                IDENTIFIER_POINTER(name));
...
tree finish_struct(tree t, tree fieldlist, tree attributes)
{
    ...
    for (x = fieldlist; x; x = TREE_CHAIN(x))
    {
       ...
       treated in some ways as volatile.  */
        ...
        if (TREE_TYPE(x) == t)
            error("nested redefinition of `%s'", IDENTIFIER_POINTER(TYPE_NAME(t)));
        ...
        if (DECL_INITIAL(x) && pedantic && TYPE_MAIN_VARIANT(TREE_TYPE(x)) != integer_type_node
            ...
            && !(TREE_CODE(TREE_TYPE(x)) == ENUMERAL_TYPE
                   && (TYPE_PRECISION(TREE_TYPE(x)) == TYPE_PRECISION(integer_type_node))))
        ...
        if (DECL_INITIAL(x))
        {
            ...
            if (TREE_CODE(TREE_TYPE(x)) == ENUMERAL_TYPE
                && (width < min_precision(TYPE_MIN_VALUE(TREE_TYPE(x)), TREE_UNSIGNED(TREE_TYPE(x)))
                       || width < min_precision(
                                      TYPE_MAX_VALUE(TREE_TYPE(x)), TREE_UNSIGNED(TREE_TYPE(x)))))
        ...
        {
            unsigned int min_align = (DECL_PACKED(x) ? BITS_PER_UNIT : TYPE_ALIGN(TREE_TYPE(x)));
    ...
    if (TREE_CODE(t) == UNION_TYPE && TYPE_TRANSPARENT_UNION(t)
    ...
    if (current_binding_level->n_incomplete != 0)
    {
        ...
        {
            if (TREE_TYPE(decl) == t && TREE_CODE(decl) != TYPE_DECL)
            {
                layout_decl(decl, 0);
            ...
            else if (TYPE_SIZE(TREE_TYPE(decl)) == 0 && TREE_CODE(TREE_TYPE(decl)) == ARRAY_TYPE)
            {
...
tree build_enumerator(tree name, tree value)
{
    ...
    if (value != 0)
    {
        ...
        {
            error("enumerator value for `%s' not integer constant", IDENTIFIER_POINTER(name));
            value = 0;
        }
    }
    ...
    type = type_for_size(MAX(TYPE_PRECISION(type), TYPE_PRECISION(integer_type_node)),
        ((flag_traditional || TYPE_PRECISION(type) >= TYPE_PRECISION(integer_type_node))
            && TREE_UNSIGNED(type)));
...
int start_function(
    tree declspecs, tree declarator, tree prefix_attributes, tree attributes, int nested)
{
    tree decl1, old_decl;
    tree restype;
    int old_immediate_size_expand = immediate_size_expand;
    ...
    if (old_decl != 0 && TREE_CODE(TREE_TYPE(old_decl)) == FUNCTION_TYPE && !DECL_BUILT_IN(old_decl)
    ...
    if (warn_strict_prototypes && TYPE_ARG_TYPES(TREE_TYPE(decl1)) == 0
        && !(old_decl != 0 && TYPE_ARG_TYPES(TREE_TYPE(old_decl)) != 0))
        warning("function declaration isn't a prototype");
    /* Optionally warn of any global def with no previous prototype.  */
    else if (warn_missing_prototypes && TREE_PUBLIC(decl1)
        && !(old_decl != 0 && TYPE_ARG_TYPES(TREE_TYPE(old_decl)) != 0)
        && strcmp("main", IDENTIFIER_POINTER(DECL_NAME(decl1))))
        warning_with_decl(decl1, "no previous prototype for `%s'");
    /* Optionally warn of any def with no previous prototype
       if the function has already been used.  */
    else if (warn_missing_prototypes && old_decl != 0 && TREE_USED(old_decl)
        && TYPE_ARG_TYPES(TREE_TYPE(old_decl)) == 0)
        warning_with_decl(decl1, "`%s' was used with no prototype before its definition");
    /* Optionally warn of any global def with no previous declaration.  */
    else if (warn_missing_declarations && TREE_PUBLIC(decl1) && old_decl == 0
        && strcmp("main", IDENTIFIER_POINTER(DECL_NAME(decl1))))
        warning_with_decl(decl1, "no previous declaration for `%s'");
    /* Optionally warn of any def with no previous declaration
       if the function has already been used.  */
    else if (warn_missing_declarations && old_decl != 0 && TREE_USED(old_decl)
        && old_decl == IDENTIFIER_IMPLICIT_DECL(DECL_NAME(decl1)))
    ...
    if (C_PROMOTING_INTEGER_TYPE_P(restype))
    {
       ...
       or if not really getting wider.  */
        if (TREE_UNSIGNED(restype)
            && (flag_traditional || (TYPE_PRECISION(restype) == TYPE_PRECISION(integer_type_node))))
            restype = unsigned_type_node;
        else
            restype = integer_type_node;
    }
    DECL_RESULT(current_function_decl) = build_decl(RESULT_DECL, NULL_TREE, restype);
    ...
    else
    {
       ...
       into the TREE_PURPOSE slots.  */
        ...
        for (parm = specparms; parm; parm = TREE_CHAIN(parm))
        {
            ...
            for (tail = parmdecls; tail; tail = TREE_CHAIN(tail))
                if (DECL_NAME(tail) == TREE_VALUE(parm) && TREE_CODE(tail) == PARM_DECL)
                {
                    found = tail;
                    break;
                }
       ...
       the type previously specified, and report any mismatches.  */
        ...
        if (TYPE_ARG_TYPES(TREE_TYPE(fndecl)))
        {
            ...
            {
                if (parm == 0 || type == 0 || TYPE_MAIN_VARIANT(TREE_VALUE(type)) == void_type_node)
                {
               ...
               with that declared for the arg.  */
                ...
                {
                    if (TYPE_MAIN_VARIANT(TREE_TYPE(parm)) == TYPE_MAIN_VARIANT(TREE_VALUE(type)))
                    {
                       ...
                       useful for argument types like uid_t.  */
                        ...
                        if ((TREE_CODE(TREE_TYPE(parm)) == INTEGER_TYPE
                            ...
                            && TYPE_PRECISION(TREE_TYPE(parm)) < TYPE_PRECISION(integer_type_node))
                        ...
                        if (pedantic)
                        {
                            pedwarn("promoted argument `%s' doesn't match prototype",
                                IDENTIFIER_POINTER(DECL_NAME(parm)));
                            warning_with_file_and_line(current_function_prototype_file,
                                current_function_prototype_line,
                                "prototype declaration");
                        }
                    }
        ...
        else
        {
            ...
            {
                type = perm_tree_cons(NULL_TREE, DECL_ARG_TYPE(parm), NULL_TREE);
                if (last)
...
void finish_function(int nested)
{
    ...
    if (warn_missing_noreturn && !TREE_THIS_VOLATILE(fndecl) && !current_function_returns_null
    ...
    /* With just -W, complain only if function returns both with
       and without a value.  */
    else if (extra_warnings && current_function_returns_value && current_function_returns_null)
    ...
    if (warn_larger_than && !DECL_EXTERNAL(fndecl) && TREE_TYPE(fndecl))
    {
        ...
        if (ret_type)
        {
            ...
            {
                unsigned units = TREE_INT_CST_LOW(ret_type_size) / BITS_PER_UNIT;
...
void push_c_function_context(void)
{
    struct c_function *p = (struct c_function *)xmalloc(sizeof(struct c_function));
...
void pop_c_function_context(void)
{
...
void copy_lang_decl(tree node ATTRIBUTE_UNUSED) {}
