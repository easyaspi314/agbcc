...
void c_expand_start_cond(tree cond, int exitflag, int compstmt_count)
{
...
void c_expand_start_else(void)
{
    /* An ambiguous else warning must be generated for the enclosing if
       statement, unless we see an else branch for that one, too.  */
    if (warn_parentheses && if_stack_pointer > 1
...
static void declare_hidden_char_array(const char *name, const char *value)
{
    ...
    if (TREE_INT_CST_LOW(TYPE_MAX_VALUE(TYPE_DOMAIN(type))) < vlen || warn_larger_than)
        type = build_array_type(char_type_node, build_index_type(build_int_2(vlen, 0)));
...
tree combine_strings(tree strings)
{
       ...
       for any individual strings that are not wide.  */
        ...
        for (t = strings; t; t = TREE_CHAIN(t))
        {
            int len = (TREE_STRING_LENGTH(t)
                - ((TREE_TYPE(t) == wchar_array_type_node) ? wchar_bytes : 1));
    ...
    if (flag_const_strings && (!flag_traditional && !flag_writable_strings))
    {
        tree elements = build_type_variant(wide_flag ? wchar_type_node : char_type_node, 1, 0);
        TREE_TYPE(value) = build_array_type(elements, build_index_type(build_int_2(nchars - 1, 0)));
    }
    else
        TREE_TYPE(value) = build_array_type(wide_flag ? wchar_type_node : char_type_node,
...
void decl_attributes(tree node, tree attributes, tree prefix_attributes)
{
    tree decl = 0, type = 0;
    int is_type = 0;
    tree a;
    ...
    for (a = attributes; a; a = TREE_CHAIN(a))
    {
        ...
        if (i == attrtab_idx)
        {
            if (!valid_machine_attribute(name, args, decl, type))
                warning("`%s' attribute directive ignored", IDENTIFIER_POINTER(name));
            else if (decl != 0)
                type = TREE_TYPE(decl);
            continue;
        }
        ...
        {
            warning("`%s' attribute does not apply to types", IDENTIFIER_POINTER(name));
            continue;
        }
        else if (list_length(args) < attrtab[i].min || list_length(args) > attrtab[i].max)
        {
            error(
                "wrong number of arguments specified for `%s' attribute", IDENTIFIER_POINTER(name));
            continue;
        }
        ...
        case A_NORETURN:
            if (TREE_CODE(decl) == FUNCTION_DECL)
                ...
                TREE_TYPE(decl) = type = build_pointer_type(
                    build_type_variant(TREE_TYPE(type), TREE_READONLY(TREE_TYPE(type)), 1));
            else
                warning("`%s' attribute ignored", IDENTIFIER_POINTER(name));
            break;
        ...
        case A_UNUSED:
            if (is_type)
                TREE_USED(type) = 1;
            else if (TREE_CODE(decl) == PARM_DECL || TREE_CODE(decl) == VAR_DECL
                || TREE_CODE(decl) == FUNCTION_DECL || TREE_CODE(decl) == LABEL_DECL)
                TREE_USED(decl) = 1;
            else
                warning("`%s' attribute ignored", IDENTIFIER_POINTER(name));
            break;
        ...
        case A_CONST:
            if (TREE_CODE(decl) == FUNCTION_DECL)
                ...
                TREE_TYPE(decl) = type = build_pointer_type(
                    build_type_variant(TREE_TYPE(type), 1, TREE_THIS_VOLATILE(TREE_TYPE(type))));
            else
                warning("`%s' attribute ignored", IDENTIFIER_POINTER(name));
            break;

        case A_T_UNION:
            if (is_type && TREE_CODE(type) == UNION_TYPE
                && (decl == 0
                       || (TYPE_FIELDS(type) != 0
                              && TYPE_MODE(type) == DECL_MODE(TYPE_FIELDS(type)))))
                TYPE_TRANSPARENT_UNION(type) = 1;
            else if (decl != 0 && TREE_CODE(decl) == PARM_DECL && TREE_CODE(type) == UNION_TYPE
                && TYPE_MODE(type) == DECL_MODE(TYPE_FIELDS(type)))
        ...
        case A_MODE:
            ...
            else
            {
                ...
                if (len > 4 && p[0] == '_' && p[1] == '_' && p[len - 1] == '_' && p[len - 2] == '_')
                {
               ...
               First check for the special modes.  */
                ...
                else if (0 == (typefm = type_for_mode(mode, TREE_UNSIGNED(type))))
        ...
        case A_SECTION:
                ...
                && TREE_CODE(TREE_VALUE(args)) == STRING_CST)
            {
                if (TREE_CODE(decl) == VAR_DECL && current_function_decl != NULL_TREE
                    && !TREE_STATIC(decl))
                    error_with_decl(
                        decl, "section attribute cannot be specified for local variables");
        ...
        {
            tree align_expr
                = (args ? TREE_VALUE(args) : size_int(BIGGEST_ALIGNMENT / BITS_PER_UNIT));
            int align;

            /* Strip any NOPs of any kind.  */
            while (TREE_CODE(align_expr) == NOP_EXPR || TREE_CODE(align_expr) == CONVERT_EXPR
        ...
        case A_NO_CHECK_MEMORY_USAGE:
            ...
            {
                error_with_decl(
                    decl, "`%s' attribute applies only to functions", IDENTIFIER_POINTER(name));
            }
            else if (DECL_INITIAL(decl))
            {
                error_with_decl(
                    decl, "can't set `%s' attribute after definition", IDENTIFIER_POINTER(name));
            }
            else
...
void split_specs_attrs(tree specs_attrs, tree *declspecs, tree *prefix_attributes)
{
    ...
    if (specs_attrs != NULL_TREE && TREE_CODE(specs_attrs) != TREE_LIST)
    {
...
void overflow_warning(tree value)
{
    if ((TREE_CODE(value) == INTEGER_CST
            || (TREE_CODE(value) == COMPLEX_CST && TREE_CODE(TREE_REALPART(value)) == INTEGER_CST))
        && TREE_OVERFLOW(value))
    {
...
{
    if (TREE_CODE(operand) == INTEGER_CST && TREE_CODE(TREE_TYPE(result)) == INTEGER_TYPE
        && TREE_UNSIGNED(TREE_TYPE(result)) && skip_evaluation == 0
    ...
    if (TREE_CODE(t) == INTEGER_CST)
    {
        if (TREE_OVERFLOW(t))
        {
            ...
            if (!(TREE_UNSIGNED(type) < TREE_UNSIGNED(TREE_TYPE(expr))
                ...
                if ((pedantic || TREE_UNSIGNED(type) || !int_fits_type_p(expr, unsigned_type(type)))
                    && skip_evaluation == 0)
...
void c_expand_expr_stmt(tree expr)
{
    ...
    if (TREE_TYPE(expr) != error_mark_node && TYPE_SIZE(TREE_TYPE(expr)) == 0
...
tree check_case_value(tree value)
{
    ...
    if (TREE_CODE(value) != INTEGER_CST && value != error_mark_node)
    {
...
int min_precision(tree value, int unsignedp)
{
    int log;
    ...
    if (integer_zerop(value))
        ...
        log = 32 + floor_log2(TREE_INT_CST_HIGH(value));
    else
...
tree shorten_compare(tree *op0_ptr, tree *op1_ptr, tree *restype_ptr, enum tree_code *rescode_ptr)
{
    ...
    if (!real1 && !real2 && TREE_CODE(primop1) == INTEGER_CST
        ...
        && TYPE_PRECISION(TREE_TYPE(primop1)) < TYPE_PRECISION(*restype_ptr))
    {
        ...
        type = signed_or_unsigned_type(unsignedp0 || TREE_UNSIGNED(*restype_ptr), type);
    ...
    else
    {
       ...
       using the args exactly as we received them.  */
        ...
        if (!real1 && !real2 && integer_zerop(primop1) && TREE_UNSIGNED(*restype_ptr))
        {
            tree value = 0;
            switch (code)
            {
            ...
            if (value != 0)
            {
                /* Don't forget to evaluate PRIMOP0 if it has side effects.  */
                if (TREE_SIDE_EFFECTS(primop0))
                    return build(COMPOUND_EXPR, TREE_TYPE(value), primop0, value);
                return value;
            }
        }
    }
...
tree truthvalue_conversion(tree expr)
{
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    case ADDR_EXPR:
       ...
       if it is weak, so we cannot optimize.  */
        ...
        if (TREE_SIDE_EFFECTS(TREE_OPERAND(expr, 0)))
            return build(
                COMPOUND_EXPR, boolean_type_node, TREE_OPERAND(expr, 0), boolean_true_node);
        else
            return boolean_true_node;

    case COMPLEX_EXPR:
        return build_binary_op(
            (TREE_SIDE_EFFECTS(TREE_OPERAND(expr, 1)) ? TRUTH_OR_EXPR : TRUTH_ORIF_EXPR),
    ...
    case MINUS_EXPR:
       ...
       this case.  */
        if (TARGET_FLOAT_FORMAT == IEEE_FLOAT_FORMAT && TREE_CODE(TREE_TYPE(expr)) == REAL_TYPE)
            break;
        /* fall through...  */
    case BIT_XOR_EXPR:
       ...
       two objects.  */
        if (TREE_TYPE(TREE_OPERAND(expr, 0)) == TREE_TYPE(TREE_OPERAND(expr, 1)))
            return build_binary_op(NE_EXPR, TREE_OPERAND(expr, 0), TREE_OPERAND(expr, 1), 1);
        return build_binary_op(NE_EXPR, TREE_OPERAND(expr, 0),
            fold(build1(NOP_EXPR, TREE_TYPE(TREE_OPERAND(expr, 0)), TREE_OPERAND(expr, 1))), 1);
    ...
    if (TREE_CODE(TREE_TYPE(expr)) == COMPLEX_TYPE)
    {
        ...
        return (build_binary_op((TREE_SIDE_EFFECTS(expr) ? TRUTH_OR_EXPR : TRUTH_ORIF_EXPR),
...
char *get_directive_line(FILE *finput)
{
    ...
    for (;;)
    {
        ...
        *p++ = c;
        ...
        else
        {
            if (c == '\'' || c == '"')
                looking_for
                    = c; /* Don't stop buffering until we see another one of these (or an EOF).  */
        }
...
tree c_build_qualified_type(tree type, int type_quals)
{
    ...
    if ((type_quals & TYPE_QUAL_RESTRICT)
        && (!POINTER_TYPE_P(type) || !C_TYPE_OBJECT_OR_INCOMPLETE_P(TREE_TYPE(type))))
    {
...
void c_apply_type_quals_to_decl(int type_quals, tree decl)
{
    ...
    if (type_quals & TYPE_QUAL_RESTRICT)
    {
        if (!TREE_TYPE(decl) || !POINTER_TYPE_P(TREE_TYPE(decl))
        ...
        else if (flag_strict_aliasing)
        {
            ...
            int pointed_to_alias_set = get_alias_set(TREE_TYPE(TREE_TYPE(decl)));
...
static tree c_find_base_decl(tree t)
{
    int i;
    tree decl;
    ...
    if (TREE_CODE(t) == FIELD_DECL || TREE_CODE(t) == PARM_DECL || TREE_CODE(t) == VAR_DECL)
...
int c_get_alias_set(tree t)
{
    ...
    type = (TREE_CODE_CLASS(TREE_CODE(t)) == 't') ? t : TREE_TYPE(t);
    ...
    for (u = t; TREE_CODE(u) == COMPONENT_REF || TREE_CODE(u) == ARRAY_REF; u = TREE_OPERAND(u, 0))
        if (TREE_CODE(u) == COMPONENT_REF && TREE_CODE(TREE_TYPE(TREE_OPERAND(u, 0))) == UNION_TYPE)
            return 0;
    ...
    if (TREE_CODE(t) == COMPONENT_REF && DECL_BIT_FIELD_TYPE(TREE_OPERAND(t, 1)))
