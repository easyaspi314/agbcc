...
const char tree_code_type[MAX_TREE_CODES] = {
#include "tree.def"
};
#undef DEFTREECODE
...
const char *tree_node_kind_names[] = { "decls", "types", "blocks", "stmts", "refs", "exprs", "constants",
    "identifiers", "op_identifiers", "perm_tree_lists", "temp_tree_lists", "vecs", "random kinds",
    "lang_decl kinds", "lang_type kinds" };
...
void init_obstacks(void)
{
    ...
    maybepermanent_firstobj = (char *)obstack_alloc(&maybepermanent_obstack, 0);
...
#define OBSTACK_CHUNK_FREE free
#endif
    _obstack_begin(obstack, OBSTACK_CHUNK_SIZE, 0, (void *(*)(size_t))OBSTACK_CHUNK_ALLOC,
...
void save_tree_status(struct function *p, tree context)
{
    ...
    else
    {
        ...
        {
            current = ((struct simple_obstack_stack *)xmalloc(sizeof(struct simple_obstack_stack)));
    ...
    maybepermanent_firstobj = (char *)obstack_finish(function_maybepermanent_obstack);
...
void restore_tree_status(struct function *p, tree context)
{
    ...
    if (context == NULL_TREE && obstack_empty_p(function_maybepermanent_obstack))
    {
...
{
    struct obstack_stack *p = (struct obstack_stack *)obstack_alloc(
        &obstack_stack_obstack, (sizeof(struct obstack_stack)));
...
void push_obstacks_nochange(void)
{
    struct obstack_stack *p = (struct obstack_stack *)obstack_alloc(
        &obstack_stack_obstack, (sizeof(struct obstack_stack)));
...
{
    maybepermanent_firstobj = (char *)obstack_alloc(function_maybepermanent_obstack, 0);
}
...
void rtl_in_current_obstack(void)
{
...
void rtl_in_saveable_obstack(void)
{
...
{
    struct momentary_level *tem = (struct momentary_level *)obstack_alloc(
        &momentary_obstack, sizeof(struct momentary_level));
...
{
    int tem = expression_obstack == &momentary_obstack;
...
tree make_node(enum tree_code code)
{
    ...
    case 'd': /* A decl node */
       ...
       in function_obstack.  */
        if (code == PARM_DECL && obstack != &permanent_obstack)
        {
            ...
            if (context)
                obstack = find_function_data(context)->function_maybepermanent_obstack;
        }
        break;
    ...
    usual_kind:
        ...
        if (code == BIND_EXPR && obstack != &permanent_obstack)
            obstack = saveable_obstack;
        length = sizeof(struct tree_exp) + (tree_code_length[(int)code] - 1) * sizeof(char *);
        break;
    ...
    case 'c': /* a constant */
       ...
       words is machine-dependent due to varying length of int32_t,
       ...
       to varying size and alignment of `double'.  */
        ...
        if (code == INTEGER_CST)
            ...
            length = sizeof(struct tree_common) + tree_code_length[(int)code] * sizeof(char *);
        break;
    ...
    case 'd':
        if (code != FUNCTION_DECL)
            DECL_ALIGN(t) = 1;
        DECL_IN_SYSTEM_HEADER(t) = in_system_header && (obstack == &permanent_obstack);
...
tree get_identifier(const char *text)
{
    ...
    for (idp = hash_table[hi]; idp; idp = TREE_CHAIN(idp))
        if (IDENTIFIER_LENGTH(idp) == len && IDENTIFIER_POINTER(idp)[0] == text[0]
    ...
    if (warn_id_clash && do_identifier_warnings && (unsigned)len >= id_clash_len)
        for (idp = hash_table[hi]; idp; idp = TREE_CHAIN(idp))
            ...
            {
                warning("`%s' and `%s' identical in first %d characters", IDENTIFIER_POINTER(idp),
                    text, id_clash_len);
                break;
            }
    ...
    IDENTIFIER_POINTER(idp) = (char *)obstack_copy0(&permanent_obstack, text, len);
...
{
    tree_code_length[(int)IDENTIFIER_NODE] = (size - sizeof(struct tree_common)) / sizeof(tree);
}
...
tree build_int_2_wide(int32_t low, int32_t hi)
{
...
tree build_real(tree type, double d)
{
    tree v;
    int overflow = 0;
...
real_value_from_int_cst(tree type, tree i)
{
    ...
    if (!TREE_UNSIGNED(TREE_TYPE(i)))
        REAL_VALUE_FROM_INT(d, TREE_INT_CST_LOW(i), TREE_INT_CST_HIGH(i), TYPE_MODE(type));
    else
        REAL_VALUE_FROM_UNSIGNED_INT(d, TREE_INT_CST_LOW(i), TREE_INT_CST_HIGH(i), TYPE_MODE(type));
    ...
    if (TREE_INT_CST_HIGH(i) < 0 && !TREE_UNSIGNED(TREE_TYPE(i)))
    {
        ...
        e = ((double)((int32_t)1 << (32 / 2))
            * (double)((int32_t)1 << (32 / 2)));
        d *= e;
        e = (double)(uint32_t)(~TREE_INT_CST_LOW(i));
        d += e;
        d = (-d - 1.0);
    }
    else
    {
        ...
        d = (double)(uint32_t)TREE_INT_CST_HIGH(i);
        e = ((double)((int32_t)1 << (32 / 2))
            * (double)((int32_t)1 << (32 / 2)));
        d *= e;
        e = (double)(uint32_t)TREE_INT_CST_LOW(i);
...
#else
    d = REAL_VALUE_TRUNCATE(TYPE_MODE(type), real_value_from_int_cst(type, i));
#endif
...
tree build_string(int len, const char *str)
{
    ...
    TREE_STRING_POINTER(s) = (char *)obstack_copy0(saveable_obstack, str, len);
    return s;
}
...
tree build_complex(tree type, tree real, tree imag)
{
    ...
    TREE_CONSTANT_OVERFLOW(t) = TREE_CONSTANT_OVERFLOW(real) | TREE_CONSTANT_OVERFLOW(imag);
    return t;
}
...
int integer_zerop(tree expr)
{
    ...
    return ((TREE_CODE(expr) == INTEGER_CST && !TREE_CONSTANT_OVERFLOW(expr)
                && TREE_INT_CST_LOW(expr) == 0 && TREE_INT_CST_HIGH(expr) == 0)
        || (TREE_CODE(expr) == COMPLEX_CST && integer_zerop(TREE_REALPART(expr))
...
int integer_onep(tree expr)
{
    ...
    return ((TREE_CODE(expr) == INTEGER_CST && !TREE_CONSTANT_OVERFLOW(expr)
                && TREE_INT_CST_LOW(expr) == 1 && TREE_INT_CST_HIGH(expr) == 0)
        || (TREE_CODE(expr) == COMPLEX_CST && integer_onep(TREE_REALPART(expr))
               && integer_zerop(TREE_IMAGPART(expr))));
}
...
int integer_all_onesp(tree expr)
{
    ...
    if (TREE_CODE(expr) == COMPLEX_CST && integer_all_onesp(TREE_REALPART(expr))
        && integer_zerop(TREE_IMAGPART(expr)))
        return 1;

    else if (TREE_CODE(expr) != INTEGER_CST || TREE_CONSTANT_OVERFLOW(expr))
        return 0;
    ...
    if (prec >= 32)
    {
        ...
        if (shift_amount > 32)
            ...
            high_value = ((int32_t)1 << shift_amount) - 1;

        return TREE_INT_CST_LOW(expr) == -1 && TREE_INT_CST_HIGH(expr) == high_value;
    }
    else
        return TREE_INT_CST_LOW(expr) == ((int32_t)1 << prec) - 1;
}
...
int integer_pow2p(tree expr)
{
    ...
    if (TREE_CODE(expr) == COMPLEX_CST && integer_pow2p(TREE_REALPART(expr))
        && integer_zerop(TREE_IMAGPART(expr)))
        return 1;
    ...
    prec = (POINTER_TYPE_P(TREE_TYPE(expr)) ? POINTER_SIZE : TYPE_PRECISION(TREE_TYPE(expr)));
    ...
    return ((high == 0 && (low & (low - 1)) == 0) || (low == 0 && (high & (high - 1)) == 0));
}
...
int tree_log2(tree expr)
{
    ...
    return (high != 0 ? 32 + exact_log2(high) : exact_log2(low));
}
...
int real_zerop(tree expr)
{
    ...
    return ((TREE_CODE(expr) == REAL_CST && !TREE_CONSTANT_OVERFLOW(expr)
                && REAL_VALUES_EQUAL(TREE_REAL_CST(expr), dconst0))
        || (TREE_CODE(expr) == COMPLEX_CST && real_zerop(TREE_REALPART(expr))
...
int real_onep(tree expr)
{
    ...
    return ((TREE_CODE(expr) == REAL_CST && !TREE_CONSTANT_OVERFLOW(expr)
        ...
        || (TREE_CODE(expr) == COMPLEX_CST && real_onep(TREE_REALPART(expr))
               && real_zerop(TREE_IMAGPART(expr))));
}
...
{
#if 0
  tree node = make_node (TREE_LIST);
#else
    int i;
    tree node = (tree)obstack_alloc(current_obstack, sizeof(struct tree_list));
#ifdef GATHER_STATISTICS
...
int_size_in_bytes(tree type)
{
    ...
    type = TYPE_MAIN_VARIANT(type);
    t = TYPE_SIZE_UNIT(type);
    if (t == 0 || TREE_CODE(t) != INTEGER_CST || TREE_INT_CST_HIGH(t) != 0)
        return -1;
...
int staticp(tree arg)
{
    ...
    case VAR_DECL:
        return (TREE_STATIC(arg) || DECL_EXTERNAL(arg)) && !DECL_NON_ADDR_CONST_P(arg);
    ...
    case COMPONENT_REF:
        return (!DECL_BIT_FIELD(TREE_OPERAND(arg, 1)) && staticp(TREE_OPERAND(arg, 0)));
...
tree save_expr(tree expr)
{
    ...
    if (TREE_CONSTANT(t) || (TREE_READONLY(t) && !TREE_SIDE_EFFECTS(t)) || TREE_CODE(t) == SAVE_EXPR
        || TREE_CODE(t) == ERROR_MARK)
        return t;
...
tree unsave_expr_now(tree expr)
{
    ...
    case CALL_EXPR:
        ...
        if (TREE_OPERAND(expr, 1) && TREE_CODE(TREE_OPERAND(expr, 1)) == TREE_LIST)
        {
...
int contains_placeholder_p(tree exp)
{
    ...
    case 'x':
        if (code == TREE_LIST)
            return (contains_placeholder_p(TREE_VALUE(exp))
                || (TREE_CHAIN(exp) != 0 && contains_placeholder_p(TREE_CHAIN(exp))));
        break;
    ...
    case 'e':
        ...
        case CALL_EXPR:
            return (TREE_OPERAND(exp, 1) != 0 && contains_placeholder_p(TREE_OPERAND(exp, 1)));
...
tree stabilize_reference(tree ref)
{
    ...
    case INDIRECT_REF:
        result = build_nt(INDIRECT_REF, stabilize_reference_1(TREE_OPERAND(ref, 0)));
        break;

    case COMPONENT_REF:
        result = build_nt(
            COMPONENT_REF, stabilize_reference(TREE_OPERAND(ref, 0)), TREE_OPERAND(ref, 1));
        break;
    ...
    case RTL_EXPR:
        result = build1(INDIRECT_REF, TREE_TYPE(ref),
            save_expr(build1(ADDR_EXPR, build_pointer_type(TREE_TYPE(ref)), ref)));
        break;
...
tree stabilize_reference_1(tree e)
{
    ...
    case '2':
       ...
       found inside of an array reference.  So do it just once.  */
        if (code == TRUNC_DIV_EXPR || code == TRUNC_MOD_EXPR || code == FLOOR_DIV_EXPR
            || code == FLOOR_MOD_EXPR || code == CEIL_DIV_EXPR || code == CEIL_MOD_EXPR
...
tree build(enum tree_code code, tree tt, ...)
{
    ...
    if (length == 2)
    {
        ...
        if ((arg0 && TREE_SIDE_EFFECTS(arg0)) || (arg1 && TREE_SIDE_EFFECTS(arg1)))
            TREE_SIDE_EFFECTS(t) = 1;
        TREE_RAISES(t) = (arg0 && TREE_RAISES(arg0)) || (arg1 && TREE_RAISES(arg1));
    }
    else if (length == 1)
    {
        tree arg0 = va_arg(p, tree);
...
{
    struct obstack *obstack = expression_obstack;
    int length;
#ifdef GATHER_STATISTICS
    tree_node_kind kind;
#endif
...
tree build_type_attribute_variant(tree ttype, tree attribute)
{
    if (!attribute_list_equal(TYPE_ATTRIBUTES(ttype), attribute))
    {
        ...
        hashcode = TYPE_HASH(TREE_CODE(ntype)) + TYPE_HASH(TREE_TYPE(ntype))
...
int valid_machine_attribute(
    tree attr_name, tree attr_args ATTRIBUTE_UNUSED, tree decl ATTRIBUTE_UNUSED, tree type ATTRIBUTE_UNUSED)
{
    int valid = 0;
...
#ifdef VALID_MACHINE_DECL_ATTRIBUTE
    if (decl != 0 && VALID_MACHINE_DECL_ATTRIBUTE(decl, decl_attr_list, attr_name, attr_args))
    {
        tree attr = lookup_attribute(IDENTIFIER_POINTER(attr_name), decl_attr_list);
    ...
    else if (VALID_MACHINE_TYPE_ATTRIBUTE(type, type_attr_list, attr_name, attr_args))
    {
        tree attr = lookup_attribute(IDENTIFIER_POINTER(attr_name), type_attr_list);
    ...
    else if (POINTER_TYPE_P(type) && TREE_CODE(TREE_TYPE(type)) == FUNCTION_TYPE
        && VALID_MACHINE_TYPE_ATTRIBUTE(TREE_TYPE(type), type_attr_list, attr_name, attr_args))
    {
        ...
        else
        {
            ...
            inner_type = build_type_attribute_variant(inner_type, inner_attr_list);
        }
...
int is_attribute_p(const char *attr, tree ident)
{
    ...
    {
        if (ident_len == attr_len + 4 && p[0] == '_' && p[1] == '_' && p[ident_len - 2] == '_'
            && p[ident_len - 1] == '_' && strncmp(attr, p + 2, attr_len) == 0)
            return 1;
    }
    ...
    else if (a2 && !attribute_list_contained(a1, a2))
    {
        ...
        else
        {
            ...
            for (; a2; a2 = TREE_CHAIN(a2))
                if (lookup_attribute(IDENTIFIER_POINTER(TREE_PURPOSE(a2)), attributes) == NULL_TREE)
                {
                    a1 = copy_node(a2);
...
tree merge_machine_type_attributes(tree t1, tree t2)
{
...
tree merge_machine_decl_attributes(tree olddecl, tree newdecl)
{
#ifdef MERGE_MACHINE_DECL_ATTRIBUTES
    return MERGE_MACHINE_DECL_ATTRIBUTES(olddecl, newdecl);
#else
    return merge_attributes(DECL_MACHINE_ATTRIBUTES(olddecl), DECL_MACHINE_ATTRIBUTES(newdecl));
#endif
}
...
tree type_hash_lookup(int hashcode, tree type)
{
    ...
    for (h = type_hash_table[hashcode % TYPE_HASH_SIZE]; h; h = h->next)
        if (h->hashcode == hashcode && TREE_CODE(h->type) == TREE_CODE(type)
            && TREE_TYPE(h->type) == TREE_TYPE(type)
            && attribute_list_equal(TYPE_ATTRIBUTES(h->type), TYPE_ATTRIBUTES(type))
            ...
            && (TYPE_MIN_VALUE(h->type) == TYPE_MIN_VALUE(type)
                   || tree_int_cst_equal(TYPE_MIN_VALUE(h->type), TYPE_MIN_VALUE(type)))
            /* Note that TYPE_DOMAIN is TYPE_ARG_TYPES for FUNCTION_TYPE.  */
            && (TYPE_DOMAIN(h->type) == TYPE_DOMAIN(type)
                   || (TYPE_DOMAIN(h->type) && TREE_CODE(TYPE_DOMAIN(h->type)) == TREE_LIST
                          && TYPE_DOMAIN(type) && TREE_CODE(TYPE_DOMAIN(type)) == TREE_LIST
...
int attribute_hash_list(tree list)
{
    int hashcode;
    tree tail;
    for (hashcode = 0, tail = list; tail; tail = TREE_CHAIN(tail))
...
{
    return attribute_list_contained(l1, l2) && attribute_list_contained(l2, l1);
}
...
int attribute_list_contained(tree l1, tree l2)
{
    ...
    for (t1 = l1, t2 = l2;
         t1 && t2 && TREE_PURPOSE(t1) == TREE_PURPOSE(t2) && TREE_VALUE(t1) == TREE_VALUE(t2);
...
int simple_cst_equal(tree t1, tree t2)
{
    ...
    else if (code2 == NOP_EXPR || code2 == CONVERT_EXPR || code2 == NON_LVALUE_EXPR)
...
/* Create a type of integers to be the TYPE_DOMAIN of an ARRAY_TYPE.
   ...
   The maximum value that MAXVAL can have is INT_MAX for a int32_t.
   We don't enforce this limit, that is up to caller (e.g. language front end).
   The limit exists because the result is a signed type and we don't handle
   sizes that use more than one int32_t.  */
...
tree get_unwidened(tree op, tree for_type)
{
    ...
    int uns = (for_type != 0 && for_type != type && final_prec > TYPE_PRECISION(type)
        && TREE_UNSIGNED(type));
    tree win = op;
    ...
    {
        int bitschange
            = TYPE_PRECISION(TREE_TYPE(op)) - TYPE_PRECISION(TREE_TYPE(TREE_OPERAND(op, 0)));

        /* Truncations are many-one so cannot be removed.
       Unless we are later going to truncate down even farther.  */
        if (bitschange < 0 && final_prec > TYPE_PRECISION(TREE_TYPE(op)))
            break;
       ...
       are all going to be discarded later by truncating to FOR_TYPE.  */
        ...
        if (bitschange > 0)
        {
            ...
            if ((uns || TREE_CODE(op) == NOP_EXPR) && TREE_UNSIGNED(TREE_TYPE(op)))
            {
                uns = 1;
                win = op;
            }
        ...
        && DECL_SIZE(TREE_OPERAND(op, 1)) != 0)
    {
       ...
       must fit the same conditions as for other extensions.  */
        ...
        if (innerprec < TYPE_PRECISION(TREE_TYPE(op))
            ...
            && (!uns || final_prec <= innerprec || TREE_UNSIGNED(TREE_OPERAND(op, 1))) && type != 0)
        {
...
tree get_narrower(tree op, int *unsignedp_ptr)
{
        ...
        && TREE_CODE(TREE_TYPE(op)) != REAL_TYPE)
    {
       ...
       advantage in doing this.  */
        ...
        if (innerprec < TYPE_PRECISION(TREE_TYPE(op)) && !DECL_BIT_FIELD(TREE_OPERAND(op, 1))
            && (first || uns == TREE_UNSIGNED(TREE_OPERAND(op, 1))) && type != 0)
        {
            if (first)
...
int int_fits_type_p(tree c, tree type)
{
    if (TREE_UNSIGNED(type))
        ...
        return (
            !(TREE_CODE(TYPE_MAX_VALUE(type)) == INTEGER_CST && INT_CST_LT(TYPE_MAX_VALUE(type), c))
            && !(TREE_CODE(TYPE_MIN_VALUE(type)) == INTEGER_CST
...
tree get_set_constructor_bits(tree init, char *buffer, int bit_size)
{
    int i;
    tree vals;
    int32_t domain_min = TREE_INT_CST_LOW(TYPE_MIN_VALUE(TYPE_DOMAIN(TREE_TYPE(init))));
    tree non_const_bits = NULL_TREE;
    for (i = 0; i < bit_size; i++)
        buffer[i] = 0;

    for (vals = TREE_OPERAND(init, 1); vals != NULL_TREE; vals = TREE_CHAIN(vals))
    {
        if (TREE_CODE(TREE_VALUE(vals)) != INTEGER_CST
            || (TREE_PURPOSE(vals) != NULL_TREE && TREE_CODE(TREE_PURPOSE(vals)) != INTEGER_CST))
            non_const_bits = tree_cons(TREE_PURPOSE(vals), TREE_VALUE(vals), non_const_bits);
        else if (TREE_PURPOSE(vals) != NULL_TREE)
        {
            /* Set a range of bits to ones.  */
            int32_t lo_index = TREE_INT_CST_LOW(TREE_PURPOSE(vals)) - domain_min;
            int32_t hi_index = TREE_INT_CST_LOW(TREE_VALUE(vals)) - domain_min;
            if (lo_index < 0 || lo_index >= bit_size || hi_index < 0 || hi_index >= bit_size)
                abort();
        ...
        else
        {
            ...
            int32_t index = TREE_INT_CST_LOW(TREE_VALUE(vals)) - domain_min;
...
tree tree_check(tree node, enum tree_code code, char *file, int line, int nofatal)
{
...
tree tree_class_check(tree node, int cl, char *file, int line, int nofatal)
{
    if (TREE_CODE_CLASS(TREE_CODE(node)) == cl)
        ...
        fatal("%s:%d: Expect '%c', have '%s'\n", file, line, cl, tree_code_name[TREE_CODE(node)]);
}

/* Likewise, but complain if the tree node is not an expression.  */

tree expr_check(tree node, int ignored, char *file, int line, int nofatal)
{
